...
| C3 | `claude/lucid-heisenberg-ppa3kj-c3`（**已推送**，叠在 c2 上） | OpenAI 客户端（Chat / Responses）→ Anthropic provider 走 Stage 管线：入口只跑 source 半 transform，边缘转 Beta（同 BaseTransform），Tool Round Stage（Beta）→ `targetTransformStage`（Beta，每轮一次）→ Anthropic 上游；`ServeStageOpenAI` 用旧 writer（`AnthropicToOpenAIStream` / `HandleAnthropicBetaToOpenAIResponsesStream` / `ConvertAnthropicToOpenAIResponseWithProvider` / `HandleAnthropicBetaToResponses`）在出口转回，故无需入口 bridge 透传 Heartbeat，`: keep-alive` 由 sdkstream 写出；Guardrails 仍由场景开关决定（OpenAI 场景是否启用是产品决策，本分支不改），启用时 gate 自己筛请求；删除 `dispatchAnthropicBetaToOpenAIChat`、`{stream,nonstream}AnthropicBetaToResponses`、`StreamAnthropicBetaToOpenAIChatWithMCP`、`RunGenericAnthropicBetaNonStream`、`mcp_hooks.go` 等；golden 逐字节不变；M1（Responses→Anthropic）、M5 移出 known-gap；keep-alive 断言扩到所有 Stage 协议对 | OpenAI→Anthropic |
| Z | `claude/lucid-heisenberg-ppa3kj-z`（**已推送**，叠在 c3 上，2 个提交） | 清理（净删约 4100 行，范围 = 相对 main 新增的死代码）：Z1 删 Anthropic 旧工具循环（toolengine V1/Beta adapter 与 forwarder、`RunGenericAnthropicV1NonStream`、旧流/非流 guardrails 钩子、`ShouldUseGenericMCPForProvider`——main 上 provider limits 实际已不生效，配置字段保留）；Z2 删旧流式 guardrails（`HandleContext.Guardrails` 及改写链、累加器、拦截器 `RewriteClientEvent`）和跨协议旧 writer（Responses→Anthropic 流/组装、Chat→Anthropic 流、对应非流），转换器的语义测试改到存活的转换器上保留。**不在范围**：Chat→Chat generic MCP 循环（仍在用）、`PopOpenAIContinuationSegment`（有写入方，且与热修复分支冲突）、录制、main 上本就无引用的代码（如 `streamemit`） | — |
