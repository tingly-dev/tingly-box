...
>
> Status: **实施中**（基底 = feat/task 合入 + §10 修正）。已完成：修正 1
> （outcome 不可解析一律 needs_input，含 continue 无 follow-up 时的免费
...
**前端定位推论：board 详情页是 web attendance surface**——IM channel
的同构物（"web remote"），而不只是管理列表。它与 IM 遵循同一交互契约：
needs_input 的问答内联呈现（问题 + 回复框，回复 = 同一 session 上的
下一次 run）、run 时间线即对话轮次（你发出的 instruction / agent 的
outcome 摘要）、handoff 给接管命令。escalating prompter 落地后，
web 详情页与 IM 是同一次提问的两个投递面。同时按 §5 的原生模式原则，
边界 UI 直接使用 agent 自己的词汇（Plan / Accept Edits、
Read Only / Workspace Write）与原生 allowlist（Read · Glob · Grep …），
友好说明只出现在 description，不做跨 agent 的统一别名。
...
对照 12 条原则复盘首版 UI 后补的四件事：

- **Live activity（第一问的内容级答案）**：running 时详情内嵌活跃 run 的
  时间戳事件流（run_started / assistant / tool / outcome），随详情轮询
  自更新——"现在在跑什么"从一个 status chip 变成可读的过程；SSE 落地后
  同一区块换数据源即可。
- **空状态模板（ux #8）**：无任务时展示可点的模板卡（每日代码复查 cron、
  盯到测试全绿 keep-checking、一次性 shell 命令），点击预填创建对话框——
  教育内嵌在产品里，而不是一个空白输入框。
- **产物为一等公民（ux #11）**：outcome 的 artifacts 渲染为可复制完整
  路径的 chips——此前该字段有数据但完全没有 UI。
...
### 7.3 生命周期：启停与删除（trigger 是独立的轴）

启停 ≠ 停止（Stop=Cancel，取消当前 run 转终态）。**pause/resume 作用在
trigger 上**：暂停后调度器跳过它，但历史、schedule、session、workspace
全保留，随时 resume——正是 §6.2 修正 2 预留的"trigger 的 enabled/paused
单独建模，不压进 Task.Status"（ux #4 正交轴）。

- 模型：`Task.TriggerPaused bool`（独立列 + 索引），`FindDueTasks` 加
  `trigger_paused = false` 过滤。**不复用 Status**——避免一个字段承载
  "最近 run 状态"和"触发开关"两义。
- resume 语义：recurring 任务若在暂停期间错过了触发点，resume 时把
  `scheduled_at` 前滚到下一次，不补发一串陈旧 tick。
- 删除：`DELETE /tasks/:id` 事务删任务 + 全部 run 历史；running/queued
  必须先 Stop（`ErrNotCancellable`），绝不孤儿化在跑的进程。**磁盘
  workspace 不动**——tb 没创建用户指定的目录，就不替他删（守则：副作用
  限定在自己的表面，ux #12）；确认框把这点写明。
- 命名：run history 里 needs_input 的 chip 是 "Paused"（run 等输入），
  trigger 暂停的 chip 改为 "Trigger paused"，避免同词异义（ux #3）。
...
### 12.5 关键架构约束：steps 当前是单执行器的（重要发现）

实现时发现：`agenttask.Step` 只有 `{ID, Title, Instruction}`，**全部由
agent 执行**；shell 是独立 task 类型 `shelltask`。也就是说**当前模型下
steps 不能异构**——§4.2 那个"step1 shell 产信号 → step2 claude、
until steps.test.exit==0"的杀手用例，前提是 **step 携带自己的
executor**。所以：

- **好 loop 的完整实现 = 先做异构 steps（每 step 带 executor + 对应
  payload），再加 when/until**，这是 agenttask 的真实重构（step 执行分发、
  session 归属、结构化信号读取都要改），不是 additive 加字段。
- `when: steps.<id>.succeeded|failed` 若只在 agent-only steps 上做，
  compelling 用例（shell 当 oracle）仍不可达，会沦为半成品——因此
  **不建议在异构 steps 之前单独做 when/until**。
...
- **CRUD 补齐（与模型正交，可独立先做）**：clone 任务【已做：
  ...
  目录/自定义路径复用、cron 克隆默认暂停待审】、list 筛选/搜索【已做：
  侧栏搜索框 + executor 切换，客户端过滤已加载列表；真正的服务端
  分页留待 >200 任务的规模再做】、（可选）重跑某次历史 run【待做，
  语义与 loop 模型纠缠（游标/session），建议随 loop 重构一起做】。
  动作栏收进溢出菜单（ux #9）。
