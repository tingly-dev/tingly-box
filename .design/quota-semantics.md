...
英文前端文案统一用 `left` 标注剩余额度（如 `60% left`），避免与 `remaining` 混用；代码字段 `available`、`used` 等保持原有含义。

底层 `used` / `used_percent` 以及机器接口字段仍表示已用量，供路由和内部判定使用；历史快照也保持原始数据，不在写入时反转。
statusline 将可计量窗口写成 `Quota: … left`，余额窗口单独写成 `Balance: …`；只知道已用量、却不知道余额的窗口不冒充 Balance。
