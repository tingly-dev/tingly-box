...
| `compact_keyword` | string | app | **yes** | override | Claude Code **急速压缩唤醒关键字**。最新一条 user 消息含该关键字时，智能路由的 `agent.claude_code / wake_compact` op 命中，规则即可把请求路由到本地 XML 聚合压缩虚拟模型（`claude-code-compact` vmodel）——瞬时产出会话摘要、不耗上游 token。空 = 用 built-in 默认 `typ.DefaultCompactKeyword`（`"compact"`）。`Suggestions`（`typ.DefaultCompactKeywords()`）透出 `compact`/`compress`/`/compact` 供快选。**注入点是路由层（Type 5）而非 transform**：`SmartRoutingStage` 仅在 claude_code 场景下用 `Config.GetEffectiveCompactKeyword(rule)`（rule 显式 > scenario 默认 > built-in）解析关键字，与最新 user 文本做大小写不敏感 `Contains`，命中即置 `RequestContext.CompactWake=true`；`wake_compact` op 读它。关键字**不进 vmodel**——一旦路由到 `claude-code-compact`，该 vmodel 只要带 tools 即压缩（门控完全交给智能路由，这样自定义关键字也能端到端生效）。| 路由层 `SmartRoutingStage.detectCompactWake` → `Config.GetEffectiveCompactKeyword`（Type 5）|
...
| `compact_keyword` | ✅ | ✅ | Rule 显式设置（非空）> Scenario 默认 > built-in `"compact"`。解析点 `Config.GetEffectiveCompactKeyword`（路由层调用，`typ.ResolveCompactKeyword` 实现链）；`resolveRuleFlagsWithScenario` 也做同样的 rule←scenario 合并以保持 flags 一致 |
