...
| `compact_keyword` | string | app | **yes** | override | Claude Code **急速压缩唤醒关键字**。最新一条 user 消息含该关键字时，请求在 service selection **之前**被短路到本地 XML 聚合压缩虚拟模型（`claude-code-compact` vmodel）——瞬时产出会话摘要、不耗上游 token。空 = 用 built-in 默认 `typ.DefaultCompactKeyword`（`"compact"`）。`Suggestions`（`typ.DefaultCompactKeywords()`）透出 `compact`/`compress`/`/compact` 供快选。**注入点是前置 handler hook（Type 1b-pre 风格），不经智能路由**：与 `applyVisionProxy` 同一调用位置（`rule` 解析完之后、`routingSelector.SelectService` 之前），但语义不同——vision proxy 命中后原地改写 request 再继续走正常 service selection，`applyCompactWake` 命中后直接**跳过** `routingSelector.SelectService`，强制把 `(provider, service)` 钉死到内置 vmodel provider（`vmodel-builtin-anthropic` + `claude-code-compact` model），从而保留"零成本瞬时响应"语义（该 vmodel 是终态 `TransformModel`，不打真实上游）。关键字解析用 `Config.GetEffectiveCompactKeyword(rule)`（rule 显式 > scenario 默认 > built-in），与最新 user 文本做大小写不敏感 `Contains`；**只在 claude_code 场景生效**。关键字**不进 vmodel**——一旦短路到 `claude-code-compact`，该 vmodel 只要带 tools 即压缩（门控完全交给前置 hook，这样自定义关键字也能端到端生效）。| `Server.applyCompactWake`（internal/server/compact_wake.go），调用点 `anthropic_message.go`，紧邻 `applyVisionProxy` |
...
| `compact_keyword` | ✅ | ✅ | Rule 显式设置（非空）> Scenario 默认 > built-in `"compact"`。解析点 `Config.GetEffectiveCompactKeyword`（`typ.ResolveCompactKeyword` 实现链），调用方是前置 hook `Server.applyCompactWake`（非路由层）；`resolveRuleFlagsWithScenario` 也做同样的 rule←scenario 合并以保持 flags 一致 |
