...
| `thinking_effort` | enum (`""`/`off`/`minimal`/`low`/`medium`/`high`/`xhigh`/`max`) | reasoning | **yes** | override | 统一控制 extended thinking。effort 是主轴：OpenAI 侧原样下发 reasoning_effort（六级全部原生支持），Anthropic 侧下发 output_config.effort（Claude 4.5+ 原生）并附 budget_tokens 兜底（minimal 1K / low 4K / medium 10K / high 20K / xhigh 24K / max 32K），vendor 阶段按模型能力对 adaptive/budget/effort 三种方言互转（见 `request_anthropic_model.go`）；Gemini 目标同样吃这张表：Gemini 3 映射为 thinking_level（minimal/low/medium/high，3 Pro 只有 low/high 向上取整），Gemini 2.5 映射为 thinking_budget（flash 系列钳到 24576，见 `request_openai_gemini.go`）。空 = "By Client"（透传客户端参数）。| `ThinkingModeTransform`（Type 1b，server-domain Transform）|
