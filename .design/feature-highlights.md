...
![Agent 配置：Claude Code 一键配置，支持 Unified / Separate 模式与 Thinking、Smart Compact 等开关](../docs/images/3-claude_code.png)
...
| **使用场景示例** | **场景：让 AI 答得上"我们公司内部"的问题**<br>一家公司的 AI 助手老被吐槽："问它最新的框架用法答的是去年的旧版，问内部系统怎么对接它完全瞎编。"根因是模型只有训练时的静态知识，既不知道最新文档，也碰不到公司私有资料。<br>**用 MCP 后**：<br>① 开启内置 `web_search` / `web_fetch`，Agent 回答前先实时检索官方最新文档再作答；<br>② 接入团队自建的「知识库 MCP Server」（封装了内部 Wiki、API 文档、工单系统），Agent 就能直接查私有数据回答"我们的下单接口怎么调"；<br>③ 通过工具可见性控制，对外部协作者只暴露 web 工具、隐藏内部知识库工具，防止越权。<br>这些 MCP 能力不仅在网关侧生效，也能通过 AgentBoot SDK 完整透传给程序化驱动的 Claude Code（见 #11）。 |
