...
视觉代理需要两样东西：**一个开关** + **一个视觉 service（provider + model）**。
按其结构形态分别落在场景配置的两个位置：

### 3.1 开关 → `ScenarioFlags`

`internal/typ/type.go` 的 `ScenarioFlags` 新增一个 typed bool 字段，沿用
`smart_compact` / `clean_header` 等既有标志的模式：
...
type ScenarioFlags struct {
    ...
    // this scenario carries an image, the configured vision service describes
    // it and the request continues downstream with image blocks replaced by
    // text. Independent of smart routing's proxy_vision op.
    VisionProxy bool `json:"vision_proxy,omitempty" yaml:"vision_proxy,omitempty"`
}
```

### 3.2 视觉服务 → `ScenarioConfig.Extensions`
...
`anthropic_message_v1.go` / `anthropic_message_beta.go`）在
`determineRuleWithScenario` 解析出 rule、**调用 `SelectService` 之前**，
插入一个共享 helper：
...
func (s *Server) applyScenarioVisionProxy(
    ...
    svc := parseVisionProxyService(cfg.Extensions) // 读 Extensions["vision_proxy_service"]
    if svc == nil {
        return
    }
    // 复用现有处理器，零行为改动
...
| 文件 | 改动 |
|------|------|
| `internal/typ/type.go` | `ScenarioFlags` 加 `VisionProxy bool` 字段 |
| `internal/server/config/config.go` | `GetScenarioFlag` / `SetScenarioFlag` 的 switch 加 `"vision_proxy"` 分支（沿用 `smart_compact` 模式）；视觉 service 走已有的 `GetScenarioConfig` / `SetScenarioConfig`（Extensions 整体存取） |
| `internal/server/vision_proxy_scenario.go`（新增） | `applyScenarioVisionProxy` helper + `parseVisionProxyService(Extensions)` |
| `internal/server/server.go` | 在 `Server` 上持有 `visionProxyProcessor` 引用（`RegisterAll` 时一并赋值） |
| `internal/server/openai_chat.go` / `openai_responses.go` / `anthropic_message_v1.go` / `anthropic_message_beta.go` | `SelectService` 前调用 `applyScenarioVisionProxy` |
| `internal/server/module/scenario/*` | 复用现有 `/scenario/:scenario`（config 整体）与 `/scenario/:scenario/flag/:flag`（bool）端点；**无需新端点**。如需 swagger 类型化，补 `VisionProxyService{Provider, Model}` 模型定义 |
...
| 元素 | 实现 |
|------|------|
| `vision_proxy` 开关 | 在 `PLUGIN_FEATURES` 列表加一项 `{ key: 'vision_proxy', label: 'Vision Proxy', description: '...' }`，沿用现有 `getScenarioFlag` / `setScenarioFlag` 渲染为 Toggle |
| 视觉 service 选择器 | 开关开启时展开一个 **provider + model 二元选择器**（复用 `ModelSelectDialog`），读写 `vision_proxy_service`，经 `getScenarioConfig` / `setScenarioConfig` 存入 `Extensions` |
...
| 入站 handler | `internal/server/{openai_chat,openai_responses,anthropic_message_v1,anthropic_message_beta}.go` |
...
| 层 | 用例 |
|----|------|
| `applyScenarioVisionProxy` 单测 | 开关关 → no-op；开关开但无 service → no-op；开关开 + 有 service + 有图 → 图片块被替换为文本 |
