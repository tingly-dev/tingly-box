...
- **（新增记录）request/reply 的绑定目标应该是 `(session, target)`，不该只是 chatID**：现在的模型（`IMPrompter.pendingRequests`）按 chatID 找"这个聊天里挂起的请求"，`HandlePromptTextReply` 直接取 `pendingReqs[0]` 当"最新一条"来匹配用户的文字回复。但 `pendingRequests` 是个 `map[string]*pendingIMRequest`，`GetPendingRequestsForChat`（`imprompter.go:374-385`）只是遍历这个 map，没有按创建时间排序——Go 的 map 遍历顺序不保证，**`[0]` 现在并不是真的"最新"，一个 chat 里同时有 ≥2 个挂起请求时，下一条文字回复会随机匹配到其中一个**。代码里的注释写的是"通常同时只有一个"（`// usually there's only one at a time`），说明这是一个已知假设，不是保证过的不变量。今天因为确实通常一个 chat 同时只有一个 pending request，这个问题不太会暴露；但如果未来支持一个 chat 同时驱动多个 session——比如切换到 App 形态、或者支持多会话并行——这个假设会先失效，而且是以"悄悄答错到另一个 session"的方式失效，比现在"没反应"还隐蔽、更难排查。往前看，绑定关系应该显式化成 `(session_id, request/target) → reply`，而不是隐式地"这个 chat 里刚发生的那个"；**`reply-to`（平台原生回复）是这个绑定在支持它的平台上的具体实现手段**——Telegram/Discord/Slack/Feishu 系都有原生回复，能把"这条回复对应哪个 request"钉死，不用再猜；没有原生回复的平台（DingTalk/Weixin/WeCom/WhatsApp）还是要退回启发式匹配，但至少应该按创建时间排序，且只在真的只有一个候选时才静默接受，有多个候选时要么提示歧义、要么都不认，不能再随便挑一个。这不是本轮要做的事，先记录方向，等真的要支持多 session 并行时再落地。
