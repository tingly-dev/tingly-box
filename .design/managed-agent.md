...
## 16. 界面：按 Claude Code web / Codex 的形态重做（2026-09-11）

之前是"表单 + 卡片列表"，和 agent 产品的心智不符。改成：

| 页 | 形态 |
|---|---|
| Tasks 首页 | 页面中央一个输入框（占据视觉中心，自动聚焦），文字下方两个小控件：目录（chip，点开是已添加目录的菜单 + "Add a folder…"）、权限模式（chip 菜单）；右侧 Start（⌘↵）。下面是任务列表：一行一个任务，左侧状态点（运行中是转圈），主标题 + "目录 · N 个改动 · 时间"，只有需要注意的状态（等待输入 / 运行中 / 失败）才在行尾显示状态 chip |
| 任务详情 | 聊天布局：顶栏（返回、标题、目录、时间、token；状态；Changes 按钮带角标；停止；归档）吸顶；对话列宽 860 居中；输入框吸底、圆角、Enter 发送 / Shift+Enter 换行、权限模式 chip 在输入框内；右侧 Changes 面板桌面端有改动时自动展开、可收起，手机端是右侧抽屉 |
...
**不设并发限制**，因为本地 Claude Code 就是这样：开几个终端标签页，在同一个仓库里
跑几个 `claude`，互不影响。用真实 CLI 验证过：同一目录、两个 `--session-id` 并发，
两个都成功，各自写自己的 `~/.claude/projects/<cwd>/<id>.jsonl`——cwd 只负责分组，
会话按 id 独立。两个 agent 同时改同一批文件是不是好主意，是用户自己的判断，不是我们
该替他拦的。（曾经加过 409 限制，是错的，已移除。）

页面上把它变成信息而不是阻拦：Folders 页每个目录显示"N 个任务进行中"，任务列表每行
显示所属目录。

还在拦的只有一件事：目录里还有活动任务时不能把它从列表移除——那会让正在跑的任务失去
归属，且不是并发问题，而是不悬空引用。文案直说"先归档"。
