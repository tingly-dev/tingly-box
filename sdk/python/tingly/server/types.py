"""Request/response value types passed to the chat handler.
...
the server shapes those into Anthropic ``message`` / ``message_*`` events or
OpenAI ``chat.completion`` / ``chat.completion.chunk`` payloads, whichever
route the caller hit, so tingly-box consumes the process as an ordinary
upstream model.
"""
...
@dataclass
class ChatRequest:
    """A normalized chat request handed to the chat handler."""
