"""A tiny HTTP server (stdlib only) speaking both LLM wire protocols.

Exposes exactly what tingly-box needs to treat this process as a provider:

    POST /v1/messages            -> Anthropic message (+ SSE when stream=true)
    POST /v1/chat/completions    -> OpenAI chat.completion (+ SSE)
    GET  /v1/models              -> the served model id
    GET  /health                 -> liveness

Both routes are always served, so which one tb calls is decided entirely by
the provider's ``api_style`` on the tb side — there is nothing to configure
here. They share the same normalized :class:`ChatRequest` and the same
handler; the author's code never sees the wire-format difference, only the
response shaping differs per route.

``GET /v1/models`` is what lets tb's model-list refresh discover this server
automatically (the provider template sets ``supports_models_endpoint``), so
the model id never has to be typed by hand.

No framework dependency — uses ``http.server.ThreadingHTTPServer`` so the
server stays a single ``pip install tingly`` away. Streaming is real SSE: a
handler that returns an iterator is emitted as protocol-appropriate
chunk/event frames.
"""
...
class _Handler(BaseHTTPRequestHandler):
    ...
    server_version = "tingly-server/0.1"

    def log_message(self, fmt, *args):  # quiet by default; the author owns logging
    ...
    def _handle_chat(self, anthropic: bool):
        ...
        try:
            ...
            if anthropic:
                return self._json(
                    500, {"type": "error", "error": {"type": "api_error", "message": f"handler error: {exc}"}}
                )
            return self._json(
                500,
                {"error": {"message": f"handler error: {exc}", "type": "api_error"}},
            )
