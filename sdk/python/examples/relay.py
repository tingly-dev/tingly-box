...
then route any scenario/rule at it. @srv.responses shares the same OpenAI
URL (it's a different path, /responses, not a different base URL) — no
extra registration step for it.

Three independent handlers, one per protocol — no shared abstraction
between them (see tingly/server.py). Each gets the raw request body exactly
as the caller sent it and forwards it to tb over `.tb.chat()` (which always
...
def handle_responses(body):
    # body["input"] is a plain string for a simple text turn (Responses also
    # allows a list of structured input items — a caller sending those would
    # need its own handling here, same caveat as @srv.messages above).
    return text_of(srv.tb.chat(model=TARGET_MODEL, messages=[{"role": "user", "content": body["input"]}]))
