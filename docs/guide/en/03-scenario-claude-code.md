...
The **Plugins** row in the config card provides several scenario-level plugin dropdowns:
...
- **Reset progress** link (top-right) clears all step progress
- A **?** icon next to it opens **How routing works** — the same [first-run Direct Routing Guide](./20-routing-rules.md#first-run-guide) documented in Routing Rules & Plugins, so you can revisit it anytime without waiting for the once-per-user auto-open
- Step 2 (**Select a Model**) can be skipped like step 4 — a **Skip** button appears next to **Choose Model**
...
A collapsible node graph at the bottom (section title: **Model Rules**) showing the full routing chain for the current scenario:
...
Each profile materializes its Claude Code settings on disk under a readable `<profile-id>--<profile-name>` directory (e.g. `p1--deepseek`), instead of an opaque ID — renaming or deleting a profile keeps the artifact directory in sync.
