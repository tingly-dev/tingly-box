...
![路由图总览](../images/routing-graph-direct.png)

规则卡片中内嵌路由图，直观展示请求的流转路径。路由图有两种模式，通过右上角的切换按钮在 **直接路由** 和 **智能路由** 之间切换。
...
## 规则扩展标记（Rule Extensions）

每条规则右侧的 **Extensions** 卡片提供预制的附加标记（Flags），可在规则层面精调请求/响应行为，无需改动服务配置。

点击 Extensions 卡片或卡片上的 **+** 按钮，打开 **Flag 目录**（分类侧边栏 + 详情面板）。
...
| 标记 | Key | 类型 | 说明 |
|------|-----|------|------|
| Session affinity | `session_affinity` | 整数（秒） | 会话粘性 TTL：同一会话在 TTL 内持续命中同一服务。0 表示禁用。通过 `metadata.user_id`、`X-Tingly-Session-ID` 请求头或客户端 IP 识别会话 |
