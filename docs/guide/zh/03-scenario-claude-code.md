...
| 插件 | 说明 |
|------|------|
| **Thinking** | 控制 extended thinking 的策略：`By Client`（由 Claude Code 自行决定）/ `Force On` / `Force Off` |
| **Smart Compact** | 智能压缩对话历史，节省 Token：`Off` / `On` |
| **Clean Header** | 去除请求中的非必要 Header，避免 Provider 兼容问题：`Off` / `On` |
| **Vision Proxy** | 代理图片 URL，解决 Provider 无法访问外网图片的问题：`Off` / `On` |
| **Record** | 录制 Claude Code 会话到 Prompt 管理（需开启 Skills 实验功能）：`Off` / `On` |

---

### 2. Quick Start 引导步骤

首次使用时显示的 **4 步引导卡**（可折叠，进度持久化在浏览器本地）：

| 步骤 | 说明 |
|------|------|
| 1. Provider | 已添加 Provider（勾选后自动标记完成） |
| 2. Model | 已在转发规则中选定模型 |
| 3. Install | 安装 Claude Code CLI（提供 npm official 和 npm mirror 两个安装命令） |
| 4. Apply | 执行 Auto Config，将配置写入本地 |

步骤间可以通过 **Back / Skip / Next** 自由跳转。点击 **Reset** 可重置所有步骤进度。
...
![Zen 模式](../images/zen-claude-code.png)

路径：`/zen/claude_code`、`/zen/claude_code/profile/:profileId`

Zen 模式将 Activity Bar 精简为最小化状态（只保留当前场景图标和 More / Exit），主内容区与普通模式完全相同。适合在专用窗口或小屏幕设备上专注操作单一场景。

进入 Zen 模式的方式：
- 点击侧边栏底部的 **More** → 选择 Zen 入口
- 直接访问 `/zen/claude_code`
