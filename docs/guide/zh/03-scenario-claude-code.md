...
配置卡中部的 **Plugins** 一行提供多个场景级插件开关，均为下拉菜单：
...
- 全部完成后引导卡自动折叠
- 右上角 **Reset progress** 链接可重置全部步骤进度
- 旁边的 **?** 图标可打开 **How routing works**——与路由规则文档中「[首次使用引导](./20-routing-rules.md#首次使用引导)」是同一个 Direct Routing Guide，无需等待仅弹一次的自动引导即可随时重新查看
- 第 2 步（**Select a Model**）与第 4 步一样可跳过——**Choose Model** 旁会出现 **Skip** 按钮
...
页面底部的可折叠节点图（区块标题为 **Model Rules**），展示当前场景的完整路由链路：
...
每个 Profile 会在磁盘上以可读的 `<profile-id>--<profile-name>` 目录（如 `p1--deepseek`）落地其 Claude Code 设置，而非用不可读的内部 ID 命名——重命名或删除 Profile 时该目录会同步更新。
