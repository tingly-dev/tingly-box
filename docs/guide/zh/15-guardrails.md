...
路径：`/guardrails`、`/guardrails/groups`、`/guardrails/rules`、`/guardrails/credentials`、`/guardrails/history`
...
![防护栏策略](../images/guardrails-rules.png)

右上角 **Manage Groups** 按钮可跳转到 [策略组](#策略组guardrailsgroups)。
...
底部 **Download Management** 卡片允许从远程策略仓库下载安装额外的策略片段；安装完成后需在上方策略列表中手动启用/禁用。**Retry** 按钮可在加载失败时重新拉取注册表。

---

## Credential（`/guardrails/credentials`）

![防护栏 Credential](../images/guardrails-credential.png)

页面标题：**Protected Credentials**。将真实密钥（API Key、Token、密码等）保留在本地，在内容到达模型之前替换为别名 Token——受保护的凭证在模型可见内容中始终处于脱敏状态，仅当工具在执行时真正需要该值时，Tingly 才会在本地还原。

- **Add Credential**：手动登记一个密钥及其别名
- **Import from Credentials**：直接从 [凭证管理](./08-credentials.md) 中拉取已有的值，无需重新输入
- **Reload**：刷新列表
