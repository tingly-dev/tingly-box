...
class TestRunner:
    ...
    def _load_config_content(self, config_path: str) -> tuple[str, bool]:
        if not config_path or config_path == "default":
            return "No config file available.", False
        path = Path(config_path)
        if not path.exists():
            return f"Config file not found: {config_path}", False
        try:
            raw = path.read_text(encoding="utf-8")
            data = json.loads(raw)
            redacted = self._redact_config(data)
            return json.dumps(redacted, indent=2, ensure_ascii=True), True
        except Exception as exc:
            return f"Failed to load config: {exc}", False
...
<body>
    <div class="container">
        ...
        <div class="footer">
            ...
            <div class="config-block">{config_html}</div>
