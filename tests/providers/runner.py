...
Comprehensive test runner that executes smoke, adaptor, and differential tests.
"""
...
class TestRunner:
    ...
    def run_all_tests(self) -> TestRunResult:
        ...
        try:
            ...
            all_results.append(("Smoke Tests", result))
            ...
            self._print(f"Smoke tests failed: {e}")
            all_results.append(("Smoke Tests", TestRunResult(
                run_id=self._get_run_id(),
                timestamp=datetime.now().isoformat(),
                config_source=self.config_path or "default",
                suite_name="Smoke Tests",
        ...
        # Run proxy smoke tests
        try:
            result = self.run_proxy_smoke_tests()
            all_results.append(("Proxy Smoke Tests", result))
            total_tests += result.total_tests
            total_passed += result.passed
            total_failed += result.failed
            total_skipped += result.skipped
            ...
            self._print(f"Proxy smoke tests failed: {e}")
            all_results.append(("Proxy Smoke Tests", TestRunResult(
                run_id=self._get_run_id(),
                timestamp=datetime.now().isoformat(),
                config_source=self.config_path or "default",
                suite_name="Proxy Smoke Tests",
        ...
        # Run adaptor tests
        try:
            result = self.run_adaptor_tests()
            all_results.append(("Adaptor Tests", result))
            total_tests += result.total_tests
            total_passed += result.passed
            total_failed += result.failed
            total_skipped += result.skipped
            ...
            self._print(f"Adaptor tests failed: {e}")
            all_results.append(("Adaptor Tests", TestRunResult(
                run_id=self._get_run_id(),
                timestamp=datetime.now().isoformat(),
                config_source=self.config_path or "default",
                suite_name="Adaptor Tests",
        ...
        try:
            result = self.run_differential_tests()
            all_results.append(("Differential Tests", result))
            total_tests += result.total_tests
            total_passed += result.passed
            total_failed += result.failed
            total_skipped += result.skipped
            ...
            self._print(f"Differential tests failed: {e}")
            all_results.append(("Differential Tests", TestRunResult(
                run_id=self._get_run_id(),
                timestamp=datetime.now().isoformat(),
                config_source=self.config_path or "default",
                suite_name="Differential Tests",
...
Examples:
  ...
  python -m tests.providers.runner --all --output /path/to/results
        """,
    )
    ...
    parser.add_argument(
        "--proxy-smoke", "-p",
        action="store_true",
        help="Run proxy smoke tests",
    )
    parser.add_argument(
        "--adaptor", "-d",
        action="store_true",
        help="Run adaptor transformation tests",
    )
    parser.add_argument(
        "--differential", "-f",
        action="store_true",
        help="Run differential transformation tests",
    )
    parser.add_argument(
        "--config", "-c",
        type=str,
