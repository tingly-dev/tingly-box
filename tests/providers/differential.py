...
Tests:
1. Roundtrip transformation (OpenAI -> Anthropic -> OpenAI)
2. Comparison of transformed vs non-transformed responses
...
class DifferentialTestSuite:
    ...
    Implements the "minority is guilty" principle: when comparing multiple
    responses, the outlier is investigated rather than assuming the majority
    is correct.
    """

    def __init__(self, config: TestConfig, verbose: bool = False):
        """
        Initialize differential test suite.
    ...
    def test_roundtrip_openai_anthropic_openai(
        ...
        Sends same request through different paths and compares responses:
        1. Direct OpenAI API
        2. Through tingly-box (with Anthropic transformation)

        The "minority is guilty" principle: if one response differs,
        investigate that response rather than assuming the majority is correct.
        """
        ...
        try:
            ...
            # Method 1: Direct OpenAI request
            ...
            # Method 2: Through tingly-box (transformed)
            ...
            # The minority is guilty: identify the outlier
            if len(responses) > 2:
                # Multiple responses - find the minority
                response_hashes = [r["response"].get("id", "") for r in responses]
                hash_counts = {}
                ...
                if minority:
                    differences.append({
                        "type": "minority_response",
                        "value": minority,
                        "message": "Minority response detected - needs investigation",
                    })
    ...
    def test_anthropic_roundtrip(
        ...
        Compares direct Anthropic API response with transformed response.
        """
        test_prompt = prompt or self.config.test_prompt
        start_time = time.time()
        ...
        try:
            ...
            # Method 2: Through tingly-box (with OpenAI transformation)
    ...
    def test_multi_provider_consistency(
        ...
        Sends same request to different providers and checks if responses
        are semantically consistent. Uses the "minority is guilty" principle.
        """
        test_prompt = prompt or self.config.test_prompt
        start_time = time.time()
        ...
        try:
            ...
            # Find minority (lowest similarity)
            if similarities:
            ...
            # Determine if there's a clear minority
    ...
    def test_response_structure_equivalence(
        ...
        Ensures that transformed responses have the same structure
    ...
    def run_all_tests(self) -> DifferentialTestSuiteResult:
        ...
        Returns:
            DifferentialTestSuiteResult with all test results.
        """
        ...
        # Test roundtrip O->A->O
        self._print("Testing roundtrip OpenAI->Anthropic->OpenAI...")
        result = self.test_roundtrip_openai_anthropic_openai()
        ...
        self._print("Testing Anthropic roundtrip...")
        result = self.test_anthropic_roundtrip()
        suite_result.results.append(result)
        suite_result.total_tests += 1
        if result.passed:
            suite_result.passed += 1
        ...
        self._print("Testing multi-provider consistency...")
        result = self.test_multi_provider_consistency()
        suite_result.results.append(result)
        suite_result.total_tests += 1
        if result.passed:
            suite_result.passed += 1
        ...
        # Test response structure equivalence
        self._print("Testing response structure equivalence...")
        result = self.test_response_structure_equivalence()
        suite_result.results.append(result)
        suite_result.total_tests += 1
        if result.passed:
            suite_result.passed += 1
