...
Tests:
1. OpenAI requests through proxy to Anthropic backend
2. Anthropic requests through proxy to OpenAI backend
3. OpenAI/Anthropic to Google transformation
4. Request/response format preservation
"""
...
class AdaptorTestResult:
    ...
    source_style: str  # Request format (openai, anthropic, google)
    target_style: str  # Backend provider style
    test_type: str
    passed: bool
    message: str
    duration_ms: float
...
class AdaptorTestSuite:
    ...
    Tests the tingly-box ability to transform requests between different
    API formats (OpenAI <-> Anthropic <-> Google).
    """

    def __init__(self, config: TestConfig, verbose: bool = False):
        """
        Initialize adaptor test suite.
    ...
    def test_openai_to_anthropic_adaptor(
        ...
        Sends OpenAI-format request through proxy to Anthropic provider.
        """
        ...
        try:
            ...
            # Send through OpenAI endpoint (which routes to Anthropic backend)
            with ProxyClient(
    ...
    def test_anthropic_to_openai_adaptor(
        ...
        Sends Anthropic-format request through proxy to OpenAI provider.
        """
        test_prompt = prompt or self.config.test_prompt
        start_time = time.time()
        ...
        try:
            ...
            # Send through Anthropic endpoint (which routes to OpenAI backend)
    ...
    def test_openai_to_google_adaptor(
        ...
        try:
            ...
            # Note: This test requires proper routing rules configured
    ...
    def test_multi_turn_conversation(
        ...
        Tests that conversation history is properly maintained
    ...
    def test_system_message_handling(
        ...
        Tests that system messages are properly handled when
        transforming between OpenAI and Anthropic formats.
        """
        test_prompt = prompt or self.config.test_prompt
        start_time = time.time()
    ...
    def run_all_tests(self) -> AdaptorTestSuiteResult:
        ...
        Returns:
            AdaptorTestSuiteResult with all test results.
        """
        ...
        self._print("Testing OpenAI to Anthropic adaptation...")
        result = self.test_openai_to_anthropic_adaptor()
        ...
        self._print("Testing Anthropic to OpenAI adaptation...")
        result = self.test_anthropic_to_openai_adaptor()
        suite_result.results.append(result)
        if result.passed:
            suite_result.passed += 1
        else:
        ...
        # Test OpenAI -> Google
        self._print("Testing OpenAI to Google adaptation...")
        result = self.test_openai_to_google_adaptor()
        suite_result.results.append(result)
        if result.passed:
            suite_result.passed += 1
        else:
        ...
        # Test Anthropic -> Google
        self._print("Testing Anthropic to Google adaptation...")
        result = self.test_anthropic_to_google_adaptor()
        suite_result.results.append(result)
        if result.passed:
            suite_result.passed += 1
        else:
        ...
        self._print("Testing multi-turn conversation...")
        result = self.test_multi_turn_conversation()
        suite_result.results.append(result)
        if result.passed:
            suite_result.passed += 1
        else:
        ...
        self._print("Testing system message handling...")
        result = self.test_system_message_handling()
        suite_result.results.append(result)
        if result.passed:
            suite_result.passed += 1
        else:
