...
class BaseProviderClient(ABC):
    ...
    def _create_headers(self) -> dict:
        ...
        return {
            ...
            "User-Agent": f"Tingly-Box-Test/1.0",
        }
...
class OpenAIClient(BaseProviderClient):
    ...
    def responses(self, prompt: str, model: str = "gpt-4") -> TestResult:
        """Send responses API request (OpenAI)."""
        start_time = time.time()
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return TestResult(
                    ...
                    provider=self.name,
                    test_type="responses",
                    message="Responses API successful",
                    duration_ms=duration_ms,
                    data={
                        "id": data.get("id"),
                        "model": data.get("model"),
    ...
    def list_models(self) -> TestResult:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # Anthropic doesn't have a models endpoint in older API versions
                return TestResult(
                    success=False,
                    provider=self.name,
                    test_type="list_models",
...
class ProxyClient:
    ...
    def __init__(
        ...
        Args:
            server_url: Tingly-box server URL
...
def create_client(provider) -> BaseProviderClient:
    """
    Factory function to create appropriate client based on provider config.

    Args:
        provider: Provider config object

    Returns:
        BaseProviderClient instance
    """
    from .config import APIStyle
