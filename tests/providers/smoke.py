...
2. Chat/completions for each API style
3. Streaming and non-streaming requests
...
class SmokeTestSuite:
    ...
    def test_provider_chat(
        ...
        if not test_model:
            self._print(f"  No test model available for {provider.name}")
        ...
        try:
            ...
            # Create chat request
            if provider.api_style == APIStyle.GOOGLE:
                request = ChatRequest(
                    model=test_model,
    ...
    def test_provider_responses_api(self, provider: Provider) -> list[SmokeTestResult]:
        """Test OpenAI Responses API (if available)."""
        results = []
        ...
        self._print(f"Testing responses API for {provider.name}")
    ...
    def _get_test_model(self, provider: Provider) -> Optional[str]:
        ...
        # Return common defaults based on API style
        if provider.api_style == APIStyle.OPENAI:
            return "tingly-gpt"
        elif provider.api_style == APIStyle.ANTHROPIC:
    ...
    def run_all_tests(self, providers: Optional[list[Provider]] = None) -> SmokeTestSuiteResult:
        ...
        Args:
            providers: List of providers to test. If None, uses config providers.

        Returns:
            SmokeTestSuiteResult with all test results.
        """
        ...
        for provider in test_providers:
            ...
            # Test responses API (OpenAI only)
            if provider.api_style == APIStyle.OPENAI:
                results = self.test_provider_responses_api(provider)
                for r in results:
                    suite_result.add_result(r)
                    self._print(f"  responses_api: {'PASS' if r.passed else 'FAIL'} - {r.message}")
...
class ProxySmokeTestSuite:
    ...
    def __init__(self, config: TestConfig, verbose: bool = False):
        """
        Initialize proxy smoke test suite.
