...
await ctx.addInitScript(() => {
    ...
    // Suppress the once-per-user Direct Routing Guide overlay (captured
    // separately, deliberately unsuppressed, further down this script).
    localStorage.setItem('tb.routingGuideAutoShown', 'true');
});
// Feature flags (guardrails/mcp/skills) are now server-side, fetched via
// GET /api/v1/scenario/_global/flag/:flag — the mock has no handler for this
// route (404s silently, flags default to false). Force it true here instead
// of touching the app's mocks/handlers.ts.
await ctx.route('**/api/v1/scenario/*/flag/*', route =>
    route.fulfill({ json: { success: true, data: { value: true } } }));
...
// Rule Plugins catalog: scroll to the "Plugins" card, click, capture dialog.
// (The card reads "Plugins" in the UI — "Extensions" was this doc's stale name.)
{
    const page = await ctx.newPage();
    page.on('pageerror', err => console.error(`  [err] ${err.message.slice(0, 80)}`));
    await page.goto(`${BASE}/agent/claude_code`, { waitUntil: 'domcontentloaded', timeout: 20000 });
    await page.waitForTimeout(5000);
    const pluginsHeader = page.locator('text=/^Plugins/').first();
    await pluginsHeader.scrollIntoViewIfNeeded().catch(() => {});
    await page.waitForTimeout(600);
    await nudgeScrollUp(page, 80);
    await pluginsHeader.click();
...
// Dashboard Activity heatmap: click the "Activity" toggle in the chart pane
// (heatmap is now inline in the Dashboard, not a standalone /overview page).
await shot('dashboard-activity', '/dashboard/7d', {
    waitMs: 4000,
    action: async (page) => {
