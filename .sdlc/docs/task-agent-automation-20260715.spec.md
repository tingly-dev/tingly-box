...
Task 可选携带一个有序步骤列表。没有步骤时保持当前自由目标语义；添加步骤后，TB 每轮只下发当前步骤，Agent 在步骤内部自主规划和调用工具：

```text
Task goal
  Step 1 → bounded Run → checkpoint
  Step 2 → bounded Run → checkpoint
...
详情展示 goal、状态、最新摘要/错误、下一次运行、workspace 绝对路径、session ID 和 native resume 命令。顺序 Task 额外展示当前步骤、已完成步骤摘要和后续步骤。
...
Create request 可选增加 `steps: [{instruction}]`；服务端生成稳定 step ID 和展示标题。Create request 仍不接受 workspace path、session ID、current step 或 step outcome。
...
5. Frontend experimental extension、navigation、Task page 和 placeholder API。
6. 顺序步骤 payload、逐轮推进和 UI。
7. 真实 Claude/Codex harness：create、resume、stop。
