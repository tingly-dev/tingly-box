...
// DeepSeek's actual logo mark (from lobehub's icon set), matching the
// ClaudeThemeIcon treatment above — a real brand mark, not a generic
// placeholder icon like the Cloud icon this theme used to show.
const DeepSeekThemeIcon = ({ size = 18, sx }: ThemeIconProps) => (
    React.createElement(
        SvgIcon,
        { viewBox: '0 0 24 24', sx: { fontSize: size, ...sx } },
        React.createElement('path', {
...
export const getThemeOptions = (t: ThemeLabelResolver) => [
    ...
    { value: 'ds', label: t('layout.activityBar.ds'), renderIcon: DeepSeekThemeIcon },
