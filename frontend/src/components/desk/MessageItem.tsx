        ...
        return (
            <Typography variant="caption" color="text.secondary" sx={{fontStyle: 'italic'}}>
                {message.kind === 'approval_response'
                    ? t('desk.wasAnswered', {defaultValue: '→ {{answer}}', answer: message.content})
                    : t('desk.wasAnsweredWith', {defaultValue: '→ answered: {{answer}}', answer: message.content || '(empty)'})}
            </Typography>
        );
    }
            ...
            <Paper variant="outlined" sx={{p: 1, bgcolor: isError ? 'error.50' : 'action.hover'}}>
                <Stack direction="row" spacing={1} sx={{alignItems: "center"}}>
                    ...
                    <Typography variant="caption" color={isError ? 'error' : 'text.secondary'} sx={{fontWeight: 600}}>
                        {message.kind === 'tool_use' ? message.content : t('desk.toolResult', {defaultValue: 'Result'})}
                    </Typography>
                </Stack>
