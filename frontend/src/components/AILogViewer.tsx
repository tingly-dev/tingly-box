...
interface RequestsViewerProps {
  ...
  ) => Promise<{ total: number; requests: ModelRequestSummary[] }>;
        ...
        const sorted = [...response.requests].sort((a, b) => {
          ...
          switch (sortField) {
            ...
            case "scenario":
              ...
              break;
            case "model":
              comparison = (a.request_model || "").localeCompare(
                b.request_model || "",
              );
              break;
            case "provider":
  ...
  const toggleRow = async (id: string) => {
    ...
    if (!details[id]) {
      try {
        ...
        setError(
          e instanceof Error ? e.message : "Failed to load request detail",
        );
      }
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          ...
          {initialScenario && (
            <Chip
              ...
              onClick={() =>
                setScenario((prev) =>
                  prev === initialScenario ? "" : initialScenario,
                )
              }
              onDelete={
                scenario === initialScenario ? () => setScenario("") : undefined
              }
        ...
        >
          One row per request. Expand for the full pipeline timeline (routing →
          conversion → upstream).
        </Typography>
      </Stack>
      {error && (
                ...
                <TableRow>
                  <TableCell colSpan={7} align="center" sx={{ py: 4 }}>
                    ...
                    >
                      {loading
                        ? "Loading..."
                        : "No model requests yet — send a request through the gateway."}
                    </Typography>
                  </TableCell>
                </TableRow>
              ) : (
                    ...
                    <Fragment key={req.request_id}>
                      ...
                      >
                        ...
                        >
                          {req.request_model || "-"}
                          {req.routed_model &&
                            req.routed_model !== req.request_model && (
                              <Typography
                                component="span"
                                sx={{
