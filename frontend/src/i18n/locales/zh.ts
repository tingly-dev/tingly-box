export default {
  ...
  "bench": {
    ...
    "requestQ": "客户端发出的是什么",
    "presetHint": "预设请求：一条消息，由工具 / 视觉 / 思考三个旋钮塑形——它就是探针本体，只是被 materialize 成了一份请求。要发别的——多轮、图片、工具结果、provider 私有字段——把请求模式切到自定义。",
    "changeStartingPoint": "换个起点",
    "startFromPreset": "照抄预设请求",
    "startFromPresetHint": "探针本体此刻会发出的请求。",
    "rawInvalid": "不是合法 JSON：{{error}}",
    "rawHint": "这就是一个说该协议的客户端会发出的请求。探测器只补 model（Anthropic 还补 max_tokens）；流式开关仍然生效；tools、图片和 thinking 在这里自己写——工具 / 视觉 / 思考旋钮只作用于预设请求。",
    "template": {"blank": "空白", "blankDesc": "该协议形态下的一份空请求。", "multi": "多轮", "multiDesc": "user → assistant → user。任意协议对的转换保真基线。", "tool": "工具往返", "toolDesc": "工具定义、assistant 的调用和工具结果——agent 框架发出的形态。", "image": "图片", "imageDesc": "文本 + 图片的 user 消息，按该协议的表达方式。", "midsys": "会话中段 system", "midsysDesc": "messages[] 里夹一条 system——claude_code_compat 就是为这个形态而存在的。"},
    "editBody": "编辑",
    "editBodyHint": "把这份 body 拿进请求编辑器作为原始请求，随意修改。",
    "noteRaw": "你自己写的请求，按其协议直接发出；只补了 model。",
    "scopeFull": "全链路",
    "scopePinned": "钉住规则",
    "scopeFullHint": "与线上完全一致：TB 按请求里的模型名匹配规则。Journey 会显示实际命中的规则。",
    "scopePinnedHint": "用 X-Tingly-Probe-Rule 强制走这条规则，只跳过规则匹配这一步。模型名与其他规则冲突或规则未激活时使用。",
    "requestMode": "请求模式",
    "requestModeHint": "预设：探针按下面的旋钮拼出请求。自定义：你自己写——参数和预设不再适用。",
    "modePreset": "预设",
    "modeCustom": "自定义",
    "groupPresets": "预设",
    "protocolCustomHint": "只在开始写自定义请求这一刻选定一次——切换协议会把 body 换成新协议的起始模板。",
    "ruleMismatch": "TB 命中的规则与你选的不同——这就是真实客户端会落到的规则。把范围切到「钉住规则」可强制走你选的那条。",
    "subtitle": "一次探测请求，所有旋钮常驻：选目标、调请求形态、临时叠加 flag、直接改 payload。这里的一切都不会写回任何规则。",
    "run": "运行",
    "running": "运行中…",
    "runHint": "⌘ / Ctrl + Enter",
    "runs": "记录",
    "runsEmpty": "本次会话的运行记录会显示在这里——点击可回看结果，并恢复当时的配置。",
    "compose": "Compose",
    "composeQ": "我要发什么？",
    "target": "Target",
    "targetPlaceholder": "搜索规则与 provider…",
    "targetMissing": "已保存的目标不存在了——请重新选择。",
    "targetEmpty": "先选一条规则或一个 provider 模型。",
    "targetLoading": "正在加载规则与 provider…",
    "rules": "规则",
    "providers": "Provider",
    "plugins": "Plugins",
    "pluginsQ": "flag 临时叠加 · 仅本次请求",
    "overridden": "{{count}} 项已覆盖",
    "nothingOverridden": "未覆盖任何项——全部继承",
    "resetAll": "全部重置",
    "reset": "恢复继承值",
    "inherit": "继承：{{value}}",
    "inheritFrom": "继承：{{value}}（{{source}}）",
    "overriddenRow": "本次请求覆盖",
    "sourceRule": "规则",
    "sourceScenario": "scenario",
    "pluginsDirect": "直连上游会绕过 TB。flag 是 TB 的中间件行为，在这里无法生效——把范围切回「经过 TB」再测 flag。",
    "pluginsNote": "灰色行显示该目标解析出的基线（规则 flag + scenario 继承）；高亮行会随请求的 flags 叠加发出。以响应里的 Flags 行为准。",
    "pluginsLoading": "正在加载 flag 目录…",
    "notEditable": "这类 flag 请在规则本身上编辑。",
    "result": "Result",
    "resultQ": "发生了什么？",
  ...
  "system": {
    ...
    "experimentalFeatures": {
      ...
      "enableBench": "探针测试工作台 —— 原始客户端请求、flag 覆盖与锁定路由",
      "on": "On",
      "off": "Off",
      "enabled": "已启用",
      "disabled": "已禁用 - 点击启用",
