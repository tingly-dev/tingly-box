    ...
    "routing": {
      ...
      "guide": {
        ...
        "steps": {
          ...
          "addModel": {
            "title": "2. 添加第一个模型",
            "content": "每条规则把一个请求模型映射到一个或多个服务。在空规则里点击「＋ 添加模型」，选择已连接的提供商和模型。需要为另一个请求模型单独建一条规则？用工具栏的「New Rule」。",
            "annotation": {
          ...
          "editModel": {
            "title": "3. 更换或移除模型",
            "content": "点击任意服务卡片即可编辑——换成别的模型、切换提供商，或移动到其他层级。悬停卡片会显示操作按钮；垃圾桶图标可把它从规则中移除。",
            "annotation": {
              "click": "点击卡片编辑 / 换模型",
              "remove": "悬停 → 垃圾桶图标移除"
            }
          ...
          "loadBalance": {
            "title": "4. 层级内负载均衡",
            "content": "当多个服务处于同一层级（T0）时，传入流量会在它们之间均匀分配，从而平衡负载、避免任何单个服务过载。",
            "annotation": {
              "sameTier": "同一层级 = 负载均衡",
              "services": "多个服务共享流量"
            }
          ...
          "tierFallback": {
            "title": "5. 基于层级的故障转移链",
            "content": "层级越低越先尝试：T0 是主选，若所有 T0 服务都失败，流量会级联到 T1，再到 T2，依此类推。用卡片上的上/下操作在层级间移动它，搭建故障转移链。",
            "annotation": {
              "primary": "T0 — 主服务（首先尝试）",
              "fallback": "T1 — 备选服务（T0 失败时使用）"
            }
          ...
          "smartIntro": {
            "title": "什么是智能路由？",
            "content": "智能路由让你定义自定义条件来控制哪个服务处理每个请求。可按模型名称、令牌数量、用户组或任意请求参数路由——精细控制，无需管理复杂的层级配置。",
            "annotation": {
