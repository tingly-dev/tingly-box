export default {
  ...
  "bench": {
    ...
    "requestQ": "客户端发出的是什么",
    "fixtureHint": "探测器内置的请求：一条消息，由工具 / 视觉 / 思考三个旋钮塑形。要发别的——多轮、图片、工具结果、provider 私有字段——就自己写请求。",
    "writeYourself": "自己写请求",
    "startFromBuilder": "编辑 builder 生成的请求",
    "startFromBuilderHint": "把文本替换为探测器 builder 此刻会发出的请求。",
    "backToFixture": "回到内置请求",
    "templates": "模板",
    "rawInvalid": "不是合法 JSON：{{error}}",
    "rawHint": "这就是一个说 {{protocol}} 协议的客户端会发出的请求。探测器只补 model（Anthropic 还补 max_tokens）；流式开关仍然生效；tools、图片和 thinking 在这里自己写——工具 / 视觉 / 思考旋钮只作用于内置请求。",
    "knobOwnedByRaw": "由你的原始请求决定，这个旋钮不生效。",
    "protocolFromRaw": "原始请求的协议——在请求编辑器里修改。",
    "template": {"multi": "多轮", "multiDesc": "user → assistant → user。任意协议对的转换保真基线。", "tool": "工具往返", "toolDesc": "工具定义、assistant 的调用和工具结果——agent 框架发出的形态。", "image": "图片", "imageDesc": "文本 + 图片的 user 消息，按该协议的表达方式。", "midsys": "会话中段 system", "midsysDesc": "messages[] 里夹一条 system——claude_code_compat 就是为这个形态而存在的。"},
    "editBody": "编辑",
