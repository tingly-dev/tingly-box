export default {
  ...
  "bench": {
    ...
    "requestQ": "客户端发出的是什么",
    "presetHint": "预设请求：一条消息，由工具 / 视觉 / 思考三个旋钮塑形——它就是探针本体，只是被 materialize 成了一份请求。要发别的——多轮、图片、工具结果、provider 私有字段——就自己写请求。",
    "writeYourself": "自己写请求",
    ...
    "rawHint": "这就是一个说 {{protocol}} 协议的客户端会发出的请求。探测器只补 model（Anthropic 还补 max_tokens）；流式开关仍然生效；tools、图片和 thinking 在这里自己写——工具 / 视觉 / 思考旋钮只作用于预设请求。",
    "rawProtocolSwitchHint": "切换协议会把下面的 body 换成该协议的起始模板。",
