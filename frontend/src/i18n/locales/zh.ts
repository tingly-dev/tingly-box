export default {
  ...
  "playground": {
    ...
    "client": "以谁的身份发送",
    "clientNone": "探测器本身（纯 SDK 请求）",
    "clientHint": "以真实客户端的身份发送：由 TB 内部对应该应用的客户端实现发出请求，header、会话键和 body 约定就是它实际发送的内容，TB 依赖入站特征的中间件（UA 优先级、clean_header、会话亲和）会按线上方式触发。仅限经过 TB 且为 Anthropic 协议。",
    "clientLabel": { "claude_code": "Claude Code" },
    "clientDesc": { "claude_code": "由 TB 自己的 Claude Code 客户端发出请求：真实的 header、会话键、思考默认值，以及 CLI 会带的身份与 billing system 块。" },
    "clientProtocolMismatch": "需要 {{protocol}} 协议",
    ...
    "conversationQ": "这次探测发送的对话轮次",
    "system": "System",
    "systemPlaceholder": "留空 = 探测默认的 echo 指令",
    "addTurn": "+ 轮次",
    "templates": "模板",
    "removeTurn": "删除轮次",
    "moveUp": "上移",
    "moveDown": "下移",
    "cycleRole": "点击切换角色",
    "turnPlaceholder": "这一轮的文本",
    "midSystemHint": "会话中段的 system——故意的非标准形态",
    "noTurns": "没有轮次：探测会发送内置的 fixture 消息（随工具轴变化）。添加一轮即可写自己的对话。",
    "lastTurnUser": "最后一轮必须是 user，否则探测会被拒绝。",
    "visionConflict": "视觉轴自带 fixture 轮次——关掉它才能发送自定义对话。",
    "template": {
      "multi": "多轮",
      "multiDesc": "user → assistant → user。任意协议对的转换保真基线。",
      "midsys": "会话中段 system",
      "midsysDesc": "messages[] 里夹一条 system——claude_code_compat 就是为这个形态而存在的。",
      "tool": "工具往返",
      "toolDesc": "assistant 发起工具调用 → 工具结果，配合打开工具轴。"
    },
    "result": "Result",
    "resultQ": "发生了什么？",
    ...
    "applyBody": "应用",
    "cancelEdit": "取消",
    "bodyInvalid": "不是合法 JSON：{{error}}",
    "bodyMustBeObject": "body 必须是一个 JSON 对象。",
    "bodyOverridden": "{{count}} 个字段被手动覆盖",
    "resetBody": "重置 body",
    "removeOverride": "移除覆盖",
    "addHeader": "+ Header",
    "headerName": "名称",
    ...
    "noteOverrides": "手动改动会变成覆盖项，在所有 builder 与 flag 之后应用——实际运行与本面板走同一路径，不可能不一致。",
    "buildFailed": "无法构造请求",
  ...
  "image-playground": {
    ...
    "dropReferenceImage": "拖拽图像到此处、点击浏览，或直接粘贴",
