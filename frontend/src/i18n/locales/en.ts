export default {
  ...
  "playground": {
    ...
    "client": "Send as",
    "clientNone": "Probe itself (plain SDK request)",
    "clientHint": "Send the request as a real client: TB's own client implementation for that app emits it, so the headers, session key and body conventions are whatever it sends — and TB's inbound-keyed middleware (user-agent precedence, clean_header, session affinity) fires as in production. Through TB and Anthropic protocol only.",
    "clientLabel": { "claude_code": "Claude Code" },
    "clientDesc": { "claude_code": "TB's own Claude Code client emits the request: real headers, session key, thinking default, plus the identity and billing system blocks the CLI sends." },
    "clientProtocolMismatch": "needs the {{protocol}} protocol",
    ...
    "conversationQ": "the turns this probe sends",
    "system": "System",
    "systemPlaceholder": "Empty = the probe's default echo instruction",
    "addTurn": "+ turn",
    "templates": "Templates",
    "removeTurn": "Remove turn",
    "moveUp": "Move up",
    "moveDown": "Move down",
    "cycleRole": "Click to change the role",
    "turnPlaceholder": "Text of this turn",
    "midSystemHint": "mid-conversation system — non-standard on purpose",
    "noTurns": "No turns: the probe sends its built-in fixture message (tool-aware). Add a turn to write your own conversation.",
    "lastTurnUser": "The last turn must be a user turn, or the probe is rejected.",
    "visionConflict": "The Vision axis brings its own fixture turn — turn it off to send a custom conversation.",
    "template": {
      "multi": "Multi-turn",
      "multiDesc": "user → assistant → user. Conversion-fidelity baseline for any protocol pair.",
      "midsys": "Mid-conversation system",
      "midsysDesc": "A system message inside messages[] — the shape claude_code_compat exists for.",
      "tool": "Tool round-trip",
      "toolDesc": "Assistant tool call → tool result, with the Tool axis switched on."
    },
    "result": "Result",
    ...
    "applyBody": "Apply",
    "cancelEdit": "Cancel",
    "bodyInvalid": "Not valid JSON: {{error}}",
    "bodyMustBeObject": "The body must be a JSON object.",
    "bodyOverridden": "{{count}} field(s) overridden by hand",
    ...
    "noteOverrides": "Hand edits become overrides applied after every builder and flag — by the run and by this panel alike, so the two cannot disagree.",
