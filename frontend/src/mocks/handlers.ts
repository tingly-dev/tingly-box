...
const mockTasks: any[] = [
    ...
    { id: 'task-loop', title: 'Green the test suite', goal: 'Keep fixing until the tests pass.', agent: 'claude', status: 'running', progress: 'Iteration 2', execution: { launch_profile: 'accept_edits', tools: ['files_read', 'files_write', 'terminal'] }, workspace_path: '/Users/demo/.tingly-box/tasks/task-loop/workspace', session_id: '019f6b1e-loop', follow_up: { enabled: false, delay_seconds: 300, max_wake_ups: 20 }, wake_count: 3, repeat: { until: 'steps.test.succeeded', max: 3, iteration: 1 }, steps: [ { id: 'test', title: 'go test ./...', executor: 'shell', command: 'go test ./...' }, { id: 'fix', title: 'Fix the failing tests', instruction: 'Fix the failing tests', when: 'steps.test.failed' } ], current_step: 0, step_outcomes: [], started_at: new Date(Date.now() - 120000).toISOString(), created_at: new Date(Date.now() - 600000).toISOString(), updated_at: new Date().toISOString() },
...
const mockTaskRuns: Record<string, any[]> = {
    ...
    'task-audit': [
        { id: 'run-audit-2', task_id: 'task-audit', attempt: 1, status: 'running', trigger: 'step', step_id: 'step-2', step_index: 1, instruction: 'Reproduce the highest-impact failure in the workspace.', execution: { launch_profile: 'workspace_write' }, progress: 'Agent working · 14 events', events: [{ id: 'ev-a1', kind: 'run_started', summary: 'Started unattended codex run', created_at: new Date(Date.now() - 175000).toISOString() }, { id: 'ev-a2', kind: 'assistant', summary: 'Reading failure cluster #1: timeout in provider pool refresh.', created_at: new Date(Date.now() - 120000).toISOString() }, { id: 'ev-a3', kind: 'tool', summary: 'shell: go test ./internal/loadbalance/ -run TestPoolRefresh -count=3', created_at: new Date(Date.now() - 60000).toISOString() }, { id: 'ev-a4', kind: 'assistant', summary: 'Reproduced: refresh deadlocks when the provider list is empty mid-rotation.', created_at: new Date(Date.now() - 12000).toISOString() }], started_at: new Date(Date.now() - 180000).toISOString(), created_at: new Date(Date.now() - 180000).toISOString(), updated_at: new Date().toISOString() },
    ...
    'task-loop': [
        { id: 'run-loop-3', task_id: 'task-loop', attempt: 1, status: 'running', trigger: 'step', step_id: 'test', step_index: 0, execution: { launch_profile: 'accept_edits' }, progress: 'Iteration 2 running tests', started_at: new Date(Date.now() - 60000).toISOString(), created_at: new Date(Date.now() - 60000).toISOString(), updated_at: new Date().toISOString() },
        { id: 'run-loop-2', task_id: 'task-loop', attempt: 1, status: 'succeeded', trigger: 'step', step_id: 'fix', step_index: 1, execution: { launch_profile: 'accept_edits' }, result: { state: 'done', summary: 'Fixed 3 of 4 failing tests.' }, started_at: new Date(Date.now() - 180000).toISOString(), finished_at: new Date(Date.now() - 120000).toISOString(), created_at: new Date(Date.now() - 180000).toISOString(), updated_at: new Date(Date.now() - 120000).toISOString() },
        { id: 'run-loop-1', task_id: 'task-loop', attempt: 1, status: 'failed', trigger: 'step', step_id: 'test', step_index: 0, result: { state: 'failed', summary: 'FAIL: 4 tests failing', exit_code: 1 }, started_at: new Date(Date.now() - 240000).toISOString(), finished_at: new Date(Date.now() - 200000).toISOString(), created_at: new Date(Date.now() - 240000).toISOString(), updated_at: new Date(Date.now() - 200000).toISOString() },
    ],
...
export const handlers = [
    ...
    http.patch('/api/v1/tasks/:id', async ({ params, request }) => { const task = mockTasks.find((item) => item.id === params.id); const body = await request.json() as any; if (!task) return HttpResponse.json({ error: 'not found' }, { status: 404 }); if (task.status === 'running' || task.status === 'queued') return HttpResponse.json({ error: 'task is not editable while running or queued' }, { status: 409 }); if ('goal' in body && !String(body.goal).trim()) return HttpResponse.json({ error: 'goal is required' }, { status: 400 }); if ('title' in body) task.title = String(body.title).trim(); if ('goal' in body) task.goal = String(body.goal).trim(); if (body.follow_up) task.follow_up = body.follow_up; if (body.execution) task.execution = body.execution; if (body.recurrence) task.recurrence = body.recurrence; if (body.clear_recurrence) delete task.recurrence; if (body.steps) { const completed = (task.steps || []).slice(0, (task.step_outcomes || []).length); task.steps = [...completed, ...body.steps.map((step: any, index: number) => ({ id: `step-${completed.length + index + 1}`, title: step.instruction.trim().split('\n')[0].slice(0, 80), instruction: step.instruction.trim() }))]; } task.updated_at = new Date().toISOString(); return HttpResponse.json({ data: task }) }),
    http.post('/api/v1/tasks/:id/pause', ({ params }) => { const task = mockTasks.find((item) => item.id === params.id); if (!task) return HttpResponse.json({ error: 'not found' }, { status: 404 }); task.trigger_paused = true; task.updated_at = new Date().toISOString(); return HttpResponse.json({ data: task }) }),
    http.post('/api/v1/tasks/:id/resume', ({ params }) => { const task = mockTasks.find((item) => item.id === params.id); if (!task) return HttpResponse.json({ error: 'not found' }, { status: 404 }); task.trigger_paused = false; task.updated_at = new Date().toISOString(); return HttpResponse.json({ data: task }) }),
    http.delete('/api/v1/tasks/:id', ({ params }) => { const index = mockTasks.findIndex((item) => item.id === params.id); if (index === -1) return HttpResponse.json({ error: 'not found' }, { status: 404 }); mockTasks.splice(index, 1); return new HttpResponse(null, { status: 204 }) }),
    http.post('/api/v1/tasks/:id/clone', ({ params }) => { const src = mockTasks.find((item) => item.id === params.id); if (!src) return HttpResponse.json({ error: 'not found' }, { status: 404 }); const clone = { ...src, id: `task-${Date.now()}`, title: (src.title || src.goal).slice(0, 40) + ' (copy)', status: 'pending', session_id: undefined, resume_command: undefined, wake_count: 0, current_step: 0, step_outcomes: [], latest_result: undefined, trigger_paused: Boolean(src.recurrence), created_at: new Date().toISOString(), updated_at: new Date().toISOString() }; mockTasks.unshift(clone); return HttpResponse.json({ data: clone }, { status: 201 }) }),
    http.get('/api/v1/tasks/:id/runs', ({ params }) => HttpResponse.json({ data: mockTaskRuns[String(params.id)] || [] })),
    http.post('/api/v1/tasks', async ({ request }) => { const body = await request.json() as any; const steps = (body.steps || []).map((step: any, index: number) => { const isShell = step.executor === 'shell'; const src = (isShell ? step.command : step.instruction) || ''; return { id: `step-${index + 1}`, title: src.trim().split('\n')[0].slice(0, 80), instruction: isShell ? undefined : src.trim(), command: isShell ? src.trim() : undefined, executor: isShell ? 'shell' : undefined, when: step.when || undefined }; }); const task = { ...body, steps, repeat: body.repeat, current_step: 0, step_outcomes: [], id: `task-${Date.now()}`, title: body.title || body.goal.slice(0, 48), status: 'pending', workspace_path: body.workspace_path || `/Users/demo/.tingly-box/tasks/new/workspace`, wake_count: 0, created_at: new Date().toISOString(), updated_at: new Date().toISOString() }; mockTasks.unshift(task); return HttpResponse.json({ data: task }, { status: 201 }) }),
