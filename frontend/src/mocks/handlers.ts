...
const mockTasks: any[] = [
    { id: 'task-release', title: 'Watch release checks', goal: 'Monitor the release checks and summarize anything blocking the rollout.', agent: 'claude', status: 'running', progress: 'Approval requested for Bash', active_run_id: 'run-release-4', attention: { id: 'control-release', kind: 'approval', tool_name: 'Bash', input: { command: 'gh release create v2.4.0 --generate-notes', description: 'Publish the validated release' }, created_at: new Date(Date.now() - 120000).toISOString(), expires_at: new Date(Date.now() + 480000).toISOString() }, execution: { launch_profile: 'claude_manual', tools: ['files_read', 'files_write', 'terminal'] }, workspace_path: '/Users/demo/.tingly-box/tasks/task-release/workspace', session_id: 'a27a39d1-12d1-44e1-9c5d-42dd28cf54e0', resume_command: "cd '/Users/demo/.tingly-box/tasks/task-release/workspace' && claude --resume 'a27a39d1-12d1-44e1-9c5d-42dd28cf54e0' --permission-mode 'default' --tools 'Read,Glob,Grep,Edit,Write,Bash'", follow_up: { enabled: true, delay_seconds: 300, max_wake_ups: 20 }, wake_count: 3, created_at: new Date(Date.now() - 7200000).toISOString(), updated_at: new Date(Date.now() - 120000).toISOString() },
    { id: 'task-audit', title: 'Audit API errors', goal: 'Inspect recent API failures and prepare a concise root-cause report.', agent: 'codex', status: 'running', progress: 'Agent working · 14 events', execution: { launch_profile: 'codex_workspace_write' }, workspace_path: '/Users/demo/.tingly-box/tasks/task-audit/workspace', session_id: '019f6b1e-2e7b-7000-a53c-37dcc2381aae', resume_command: "cd '/Users/demo/.tingly-box/tasks/task-audit/workspace' && codex exec -s 'workspace-write' resume '019f6b1e-2e7b-7000-a53c-37dcc2381aae'", follow_up: { enabled: false, delay_seconds: 300, max_wake_ups: 20 }, wake_count: 1, steps: [{ id: 'step-1', title: 'Inspect recent failures', instruction: 'Inspect recent API failures and group them by root cause.' }, { id: 'step-2', title: 'Reproduce the leading failure', instruction: 'Reproduce the highest-impact failure in the workspace.' }, { id: 'step-3', title: 'Prepare the report', instruction: 'Write a concise root-cause report with recommended next actions.' }], current_step: 1, step_outcomes: [{ step_id: 'step-1', result: { state: 'done', summary: 'Grouped 23 failures into three root-cause clusters.' }, completed_at: new Date(Date.now() - 240000).toISOString() }], started_at: new Date(Date.now() - 180000).toISOString(), created_at: new Date(Date.now() - 3600000).toISOString(), updated_at: new Date().toISOString() },
    { id: 'task-daily', title: 'Daily dependency review', goal: 'Review dependency updates and flag security-sensitive changes.', agent: 'codex', status: 'pending', execution: { launch_profile: 'codex_read_only' }, workspace_path: '/Users/demo/.tingly-box/tasks/task-daily/workspace', follow_up: { enabled: false, delay_seconds: 300, max_wake_ups: 20 }, wake_count: 0, scheduled_at: new Date(Date.now() + 86400000).toISOString(), recurrence: { cron: '0 9 * * *', timezone: 'Asia/Shanghai' }, created_at: new Date(Date.now() - 86400000).toISOString(), updated_at: new Date().toISOString() },
    { id: 'task-report', title: 'Generate migration notes', goal: 'Write migration notes for the completed routing refactor.', agent: 'claude', status: 'succeeded', execution: { launch_profile: 'claude_accept_edits', tools: ['files_read', 'files_write', 'terminal'] }, latest_result: { state: 'done', summary: 'Migration notes written to docs/routing-migration.md.', artifacts: ['docs/routing-migration.md'] }, workspace_path: '/Users/demo/.tingly-box/tasks/task-report/workspace', session_id: '5727aa28-d554-49e9-8770-a413659f1988', resume_command: "cd '/Users/demo/.tingly-box/tasks/task-report/workspace' && claude --resume '5727aa28-d554-49e9-8770-a413659f1988' --permission-mode 'acceptEdits'", follow_up: { enabled: false, delay_seconds: 300, max_wake_ups: 20 }, wake_count: 1, finished_at: new Date(Date.now() - 7200000).toISOString(), created_at: new Date(Date.now() - 10800000).toISOString(), updated_at: new Date(Date.now() - 7200000).toISOString() },
]

const mockTaskRuns: Record<string, any[]> = {
    'task-release': [
        { id: 'run-release-4', task_id: 'task-release', attempt: 1, status: 'waiting_approval', trigger: 'run', execution: { launch_profile: 'claude_manual', tools: ['files_read', 'files_write', 'terminal'] }, progress: 'Approval requested for Bash', pending_control: mockTasks[0].attention, events: [{ id: 'event-release-control', kind: 'control_requested', summary: 'Approval requested for Bash', created_at: new Date(Date.now() - 120000).toISOString() }], started_at: new Date(Date.now() - 300000).toISOString(), created_at: new Date(Date.now() - 300000).toISOString(), updated_at: new Date(Date.now() - 120000).toISOString() },
        { id: 'run-release-3', task_id: 'task-release', attempt: 1, status: 'rescheduled', trigger: 'run', execution: { launch_profile: 'claude_manual', tools: ['files_read', 'files_write', 'terminal'] }, result: { state: 'continue', summary: 'Release checks passed; waiting for the publish window.' }, started_at: new Date(Date.now() - 1800000).toISOString(), finished_at: new Date(Date.now() - 1680000).toISOString(), created_at: new Date(Date.now() - 1800000).toISOString(), updated_at: new Date(Date.now() - 1680000).toISOString() },
    ],
    'task-audit': [
        { id: 'run-audit-2', task_id: 'task-audit', attempt: 1, status: 'running', trigger: 'step', step_id: 'step-2', step_index: 1, instruction: 'Reproduce the highest-impact failure in the workspace.', execution: { launch_profile: 'codex_workspace_write' }, progress: 'Agent working · 14 events', started_at: new Date(Date.now() - 180000).toISOString(), created_at: new Date(Date.now() - 180000).toISOString(), updated_at: new Date().toISOString() },
        { id: 'run-audit-1', task_id: 'task-audit', attempt: 1, status: 'rescheduled', trigger: 'step', step_id: 'step-1', step_index: 0, instruction: 'Inspect recent API failures and group them by root cause.', execution: { launch_profile: 'codex_workspace_write' }, result: { state: 'done', summary: 'Grouped 23 failures into three root-cause clusters.' }, started_at: new Date(Date.now() - 420000).toISOString(), finished_at: new Date(Date.now() - 240000).toISOString(), created_at: new Date(Date.now() - 420000).toISOString(), updated_at: new Date(Date.now() - 240000).toISOString() },
    ],
    'task-report': [{ id: 'run-report-1', task_id: 'task-report', attempt: 1, status: 'succeeded', trigger: 'run', execution: { launch_profile: 'claude_accept_edits', tools: ['files_read', 'files_write', 'terminal'] }, result: { state: 'done', summary: 'Migration notes written to docs/routing-migration.md.' }, started_at: new Date(Date.now() - 7500000).toISOString(), finished_at: new Date(Date.now() - 7200000).toISOString(), created_at: new Date(Date.now() - 7500000).toISOString(), updated_at: new Date(Date.now() - 7200000).toISOString() }],
}
...
export const handlers = [
    ...
    http.get('/api/v1/tasks/agents', () => HttpResponse.json({ data: [
        { agent: 'claude', available: true, launch_profiles: ['claude_plan', 'claude_manual', 'claude_accept_edits'], default_profile: 'claude_accept_edits', tool_filtering: true, interactive_control: true },
        { agent: 'codex', available: true, launch_profiles: ['codex_read_only', 'codex_workspace_write'], default_profile: 'codex_workspace_write', tool_filtering: false, interactive_control: false },
    ] })),
    ...
    http.post('/api/v1/tasks', async ({ request }) => { const body = await request.json() as any; const steps = (body.steps || []).map((step: any, index: number) => ({ id: `step-${index + 1}`, title: step.instruction.trim().split('\n')[0].slice(0, 80), instruction: step.instruction.trim() })); const task = { ...body, steps, current_step: 0, step_outcomes: [], id: `task-${Date.now()}`, title: body.title || body.goal.slice(0, 48), status: 'pending', workspace_path: `/Users/demo/.tingly-box/tasks/new/workspace`, wake_count: 0, created_at: new Date().toISOString(), updated_at: new Date().toISOString() }; mockTasks.unshift(task); return HttpResponse.json({ data: task }, { status: 201 }) }),
    ...
    http.post('/api/v1/tasks/:id/runs/:runId/control/:controlId/respond', async ({ params, request }) => {
        const task = mockTasks.find((item) => item.id === params.id)
        const run = (mockTaskRuns[String(params.id)] || []).find((item) => item.id === params.runId)
        const body = await request.json() as any
        if (!task || !run || !task.attention || task.attention.id !== params.controlId) return HttpResponse.json({ error: 'task control is not active' }, { status: 409 })
        run.status = 'running'; run.pending_control = undefined; run.progress = 'Agent working · response delivered'; run.updated_at = new Date().toISOString()
        run.events = [...(run.events || []), { id: `event-${Date.now()}`, kind: 'control_answered', summary: body.action === 'approve' ? `Approved ${task.attention.tool_name} once` : body.action === 'answer' ? 'Answered agent question' : 'Denied agent request', created_at: new Date().toISOString() }]
        task.attention = undefined; task.progress = run.progress; task.updated_at = run.updated_at
