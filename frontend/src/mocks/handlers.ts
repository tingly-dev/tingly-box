...
const mockTaskRuns: Record<string, any[]> = {
    'task-release': [
        { id: 'run-release-4', task_id: 'task-release', attempt: 1, status: 'waiting_approval', trigger: 'run', execution: { launch_profile: 'manual', tools: ['files_read', 'files_write', 'terminal'] }, progress: 'Approval requested for Bash', pending_control: mockTasks[0].attention, events: [{ id: 'event-release-control', kind: 'control_requested', summary: 'Approval requested for Bash', created_at: new Date(Date.now() - 120000).toISOString() }], started_at: new Date(Date.now() - 300000).toISOString(), created_at: new Date(Date.now() - 300000).toISOString(), updated_at: new Date(Date.now() - 120000).toISOString() },
        { id: 'run-release-3', task_id: 'task-release', attempt: 1, status: 'rescheduled', trigger: 'run', execution: { launch_profile: 'manual', tools: ['files_read', 'files_write', 'terminal'] }, result: { state: 'continue', summary: 'Release checks passed; waiting for the publish window.' }, started_at: new Date(Date.now() - 1800000).toISOString(), finished_at: new Date(Date.now() - 1680000).toISOString(), created_at: new Date(Date.now() - 1800000).toISOString(), updated_at: new Date(Date.now() - 1680000).toISOString() },
    ],
    'task-audit': [
...
export const handlers = [
    ...
    http.get('/api/v1/tasks/agents', () => HttpResponse.json({ data: [
        { agent: 'claude', available: true, launch_profiles: ['plan', 'manual', 'accept_edits'], default_profile: 'accept_edits', tool_filtering: true, interactive_control: true },
        { agent: 'codex', available: true, launch_profiles: ['read_only', 'workspace_write'], default_profile: 'workspace_write', tool_filtering: false, interactive_control: false },
    ] })),
