...
const mockTasks: any[] = [
    ...
    { id: 'task-audit', title: 'Audit API errors', goal: 'Inspect recent API failures and prepare a concise root-cause report.', agent: 'codex', status: 'running', progress: 'Agent working · 14 events', workspace_path: '/Users/demo/.tingly-box/tasks/task-audit/workspace', session_id: '019f6b1e-2e7b-7000-a53c-37dcc2381aae', resume_command: "cd '/Users/demo/.tingly-box/tasks/task-audit/workspace' && codex exec resume '019f6b1e-2e7b-7000-a53c-37dcc2381aae'", follow_up: { enabled: false, delay_seconds: 300, max_wake_ups: 20 }, wake_count: 1, started_at: new Date(Date.now() - 180000).toISOString(), created_at: new Date(Date.now() - 3600000).toISOString(), updated_at: new Date().toISOString() },
    { id: 'task-daily', title: 'Daily dependency review', goal: 'Review dependency updates and flag security-sensitive changes.', agent: 'codex', status: 'pending', workspace_path: '/Users/demo/.tingly-box/tasks/task-daily/workspace', follow_up: { enabled: false, delay_seconds: 300, max_wake_ups: 20 }, wake_count: 0, scheduled_at: new Date(Date.now() + 86400000).toISOString(), recurrence: { cron: '0 9 * * *', timezone: 'Asia/Shanghai' }, created_at: new Date(Date.now() - 86400000).toISOString(), updated_at: new Date().toISOString() },
    { id: 'task-report', title: 'Generate migration notes', goal: 'Write migration notes for the completed routing refactor.', agent: 'claude', status: 'succeeded', latest_result: { state: 'done', summary: 'Migration notes written to docs/routing-migration.md.', artifacts: ['docs/routing-migration.md'] }, workspace_path: '/Users/demo/.tingly-box/tasks/task-report/workspace', session_id: '5727aa28-d554-49e9-8770-a413659f1988', resume_command: "cd '/Users/demo/.tingly-box/tasks/task-report/workspace' && claude --resume '5727aa28-d554-49e9-8770-a413659f1988'", follow_up: { enabled: false, delay_seconds: 300, max_wake_ups: 20 }, wake_count: 1, finished_at: new Date(Date.now() - 7200000).toISOString(), created_at: new Date(Date.now() - 10800000).toISOString(), updated_at: new Date(Date.now() - 7200000).toISOString() },
]
...
export const handlers = [
    ...
    http.get('/api/v1/tasks/agents', () => HttpResponse.json({ data: [{ agent: 'claude', available: true }, { agent: 'codex', available: true }] })),
    ...
    http.post('/api/v1/tasks', async ({ request }) => { const body = await request.json() as any; const task = { ...body, id: `task-${Date.now()}`, title: body.title || body.goal.slice(0, 48), status: 'pending', workspace_path: `/Users/demo/.tingly-box/tasks/new/workspace`, wake_count: 0, created_at: new Date().toISOString(), updated_at: new Date().toISOString() }; mockTasks.unshift(task); return HttpResponse.json({ data: task }, { status: 201 }) }),
