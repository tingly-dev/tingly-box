...
import {
  ...
  Divider, FormControl, FormControlLabel, InputLabel, MenuItem, Paper, Select, Stack, Switch,
  TextField, ToggleButton, ToggleButtonGroup, Tooltip, Typography,
} from '@mui/material';
import PageHeader from '@/components/PageHeader';
import { Add, Block, ContentCopy, PlayArrow, Refresh, Schedule, Send, Terminal } from '@/components/icons';
...
function CreateTaskDialog({ open, agents, onClose, onCreated }: {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setGoal(''); onCreated(created); onClose();
  ...
  return <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
    ...
    <DialogActions><Button onClick={onClose}>Cancel</Button><Button variant="contained" onClick={submit} disabled={saving || !goal.trim() || !availability(agent) || (when === 'later' && !scheduledAt) || (when === 'repeat' && !cron.trim())}>{saving ? <CircularProgress size={18} /> : 'Create task'}</Button></DialogActions>
  </Dialog>;
}
