...
import { Box, Button, Checkbox, Chip, IconButton, ListItemText, MenuItem, Select, Stack, Switch, TextField, Tooltip, Typography } from '@mui/material';
...
// (.design/bench.md §5.4). Every row shows the resolved baseline for the
...
const CATEGORY_ORDER = ['app', 'request', 'request_openai', 'request_anthropic', 'response', 'reasoning', 'vision', 'routing', 'observability'];
const EDITABLE_TYPES = new Set(['bool', 'string', 'int', 'enum', 'multi_enum']);

// multi_enum values are comma-separated option values ('' = none selected).
...
export function isFlagSet(spec: FlagSpec, value: unknown): boolean {
    switch (spec.type) {
        ...
        case 'multi_enum': return splitMulti(value).length > 0;
...
export function formatFlagValue(spec: FlagSpec, value: unknown): string {
    switch (spec.type) {
        ...
        case 'multi_enum': {
            ...
            return picked.length ? picked.map((v) => spec.options?.find((o) => o.value === v)?.label ?? v).join(', ') : '—';
        }
    ...
    switch (spec.type) {
        ...
        case 'multi_enum': {
            ...
            return (
                ...
                >
                    {(spec.options ?? []).map((o) => (
                        <MenuItem key={o.value} value={o.value} sx={{ fontSize: '0.8rem', py: 0 }}>
                            <Checkbox size="small" checked={picked.includes(o.value)} sx={{ p: 0.5 }} />
                            <ListItemText primary={o.label} slotProps={{ primary: { sx: { fontSize: '0.8rem' } } }} />
                        </MenuItem>
                    ))}
                </Select>
            );
        ...
        default:
            return (
                <Tooltip title={t('bench.notEditable', { defaultValue: 'This flag type is edited on the rule itself.' })}>
        ...
        <Stack spacing={1}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                <Typography variant="caption" sx={{ color: count ? 'text.primary' : 'text.secondary' }}>
                    {count
                        ? t('bench.overridden', { count, defaultValue: '{{count}} overridden' })
                        : t('bench.nothingOverridden', { defaultValue: 'nothing overridden — all inherited' })}
                </Typography>
                <Box sx={{ flex: 1 }} />
                <Button size="small" onClick={() => onChange({})} disabled={count === 0 || disabled} sx={{ minWidth: 0, fontSize: '0.72rem' }}>
                    {t('bench.resetAll', { defaultValue: 'Reset all' })}
                </Button>
            </Box>
            ...
            {loading && registry.length === 0 && (
                <Typography variant="caption" sx={{ color: 'text.secondary' }}>
                    {t('bench.pluginsLoading', { defaultValue: 'Loading the flag registry…' })}
                </Typography>
            )}
                    ...
                    <Box key={category} sx={{ mb: 1 }}>
                        ...
                        {specs.map((spec) => {
                            ...
                            const caption = overridden
                                ? t('bench.overriddenRow', { defaultValue: 'overridden for this request' })
                                : source
                                  ? t('bench.inheritFrom', { value: baseText, source: source === 'rule' ? t('bench.sourceRule', { defaultValue: 'rule' }) : t('bench.sourceScenario', { defaultValue: 'scenario' }), defaultValue: 'inherit: {{value}} ({{source}})' })
                                  : t('bench.inherit', { value: baseText, defaultValue: 'inherit: {{value}}' });
                            ...
                            return (
                                <Tooltip key={spec.key} title={spec.description} placement="left" enterDelay={600}>
                                    ...
                                    >
                                        ...
                                        <Box>
                                            {overridden && (
                                                <Tooltip title={t('bench.reset', { defaultValue: 'Reset to inherited' })}>
            ...
            <Typography variant="caption" sx={{ color: 'text.secondary', lineHeight: 1.4 }}>
                {t('bench.pluginsNote', { defaultValue: "Muted rows show the resolved baseline for this target (rule flags + scenario inheritance). Highlighted rows ride in the request's flags overlay. The response's Flags row is the authority." })}
            </Typography>
        </Stack>
    );
};
