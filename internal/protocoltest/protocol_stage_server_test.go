...
func TestServerProtocolStageSelection(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "default responses route legacy", source: protocol.TypeOpenAIResponses, target: protocol.TypeOpenAIResponses, wantHeader: "legacy"},
		{
		...
		{
			name:       "default v1 Guardrail route remains legacy",
			opts:       []TestEnvOption{NewTestEnvOptionWithGuardrails(NewAllowGuardrailsRuntime())},
			source:     protocol.TypeAnthropicV1,
			target:     protocol.TypeOpenAIChat,
			wantHeader: "legacy",
		...
		{
			name:       "default responses Guardrail route remains legacy",
			opts:       []TestEnvOption{NewTestEnvOptionWithGuardrails(NewAllowGuardrailsRuntime())},
		...
		{
			name:         "stage v1 promotes Guardrail request to beta",
		...
		{
			name:       "stage v1 runs Guardrail through beta to chat",
			opts:       []TestEnvOption{NewTestEnvOptionWithProtocolStage(), NewTestEnvOptionWithGuardrails(NewAllowGuardrailsRuntime())},
			source:     protocol.TypeAnthropicV1,
			target:     protocol.TypeOpenAIChat,
			wantHeader: "stage",
		...
		{
			name:       "stage v1 runs Guardrail through beta to responses",
			opts:       []TestEnvOption{NewTestEnvOptionWithProtocolStage(), NewTestEnvOptionWithGuardrails(NewAllowGuardrailsRuntime())},
			source:     protocol.TypeAnthropicV1,
		...
		{
			name:       "stage responses runs Guardrail through beta to responses",
			opts:       []TestEnvOption{NewTestEnvOptionWithProtocolStage(), NewTestEnvOptionWithGuardrails(NewAllowGuardrailsRuntime())},
			source:     protocol.TypeOpenAIResponses,
			target:     protocol.TypeOpenAIResponses,
			wantHeader: "stage",
		...
		{
			name:       "stage responses runs Guardrail through beta",
			opts:       []TestEnvOption{NewTestEnvOptionWithProtocolStage(), NewTestEnvOptionWithGuardrails(NewAllowGuardrailsRuntime())},
			source:     protocol.TypeOpenAIResponses,
			target:     protocol.TypeAnthropicBeta,
			wantHeader: "stage",
		...
		{
			name:       "stage responses runs Guardrail through beta to chat",
			opts:       []TestEnvOption{NewTestEnvOptionWithProtocolStage(), NewTestEnvOptionWithGuardrails(NewAllowGuardrailsRuntime())},
			source:     protocol.TypeOpenAIResponses,
			target:     protocol.TypeOpenAIChat,
			wantHeader: "stage",
...
func TestServerProtocolStageRecordingPreservesOriginalAnthropicInput(t *testing.T) {
	for _, source := range []protocol.APIType{protocol.TypeAnthropicV1, protocol.TypeAnthropicBeta} {
		...
		t.Run(string(source), func(t *testing.T) {
			...
			input["client_extension"] = "preserve-me"
			...
			recordedBody := string(records[0].InputRequest.Body)
			if !strings.Contains(recordedBody, `"client_extension":"preserve-me"`) {
				t.Fatalf("input request lost original extension: %s", recordedBody)
			}
		})
	}
}
...
func TestServerProtocolStageAnthropicV1GuardrailComplete(t *testing.T) {
	t.Parallel()

	for _, target := range []protocol.APIType{protocol.TypeAnthropicV1, protocol.TypeOpenAIChat, protocol.TypeOpenAIResponses} {
		target := target
		t.Run(string(target), func(t *testing.T) {
			t.Parallel()
			runtime := newProtocolStageGuardrails(func(_ context.Context, input guardrailscore.Input) (guardrailscore.Result, error) {
			...
			env := NewTestEnv(t, NewTestEnvOptionWithProtocolStage(), NewTestEnvOptionWithGuardrails(runtime))
			scenario := TextScenario()
			env.SetupRoute(protocol.TypeAnthropicV1, target, scenario)
			model := env.findRouteModel(protocol.TypeAnthropicV1, target, scenario.Name)
...
func TestServerProtocolStageAnthropicV1GuardrailStream(t *testing.T) {
	...
	for _, target := range []protocol.APIType{protocol.TypeAnthropicV1, protocol.TypeOpenAIChat, protocol.TypeOpenAIResponses} {
		...
		t.Run(string(target), func(t *testing.T) {
			...
			env.SetupRoute(protocol.TypeAnthropicV1, target, scenario)
			model := env.findRouteModel(protocol.TypeAnthropicV1, target, scenario.Name)
			path, body := buildRequest(protocol.TypeAnthropicV1, model, true)
...
func TestServerProtocolStageOpenAIResponsesGuardrailComplete(t *testing.T) {
	t.Parallel()

	for _, target := range []protocol.APIType{protocol.TypeOpenAIResponses, protocol.TypeAnthropicBeta, protocol.TypeOpenAIChat} {
		target := target
		t.Run(string(target), func(t *testing.T) {
			t.Parallel()
			runtime := newProtocolStageGuardrails(func(_ context.Context, input guardrailscore.Input) (guardrailscore.Result, error) {
			...
			env := NewTestEnv(t, NewTestEnvOptionWithProtocolStage(), NewTestEnvOptionWithGuardrails(runtime))
			scenario := TextScenario()
			env.SetupRoute(protocol.TypeOpenAIResponses, target, scenario)
			model := env.findRouteModel(protocol.TypeOpenAIResponses, target, scenario.Name)
			path, body := buildRequest(protocol.TypeOpenAIResponses, model, false)
...
func TestServerProtocolStageOpenAIResponsesGuardrailStream(t *testing.T) {
	t.Parallel()
	...
	for _, target := range []protocol.APIType{protocol.TypeOpenAIResponses, protocol.TypeAnthropicBeta, protocol.TypeOpenAIChat} {
		...
		t.Run(string(target), func(t *testing.T) {
			...
			env.SetupRoute(protocol.TypeOpenAIResponses, target, scenario)
			model := env.findRouteModel(protocol.TypeOpenAIResponses, target, scenario.Name)
			path, body := buildRequest(protocol.TypeOpenAIResponses, model, true)
