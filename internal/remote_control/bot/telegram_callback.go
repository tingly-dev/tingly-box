...
func (h *BotHandler) handlePermissionCallback(hCtx HandlerContext, parts []string) {
	...
	case "always_exact", "always_glob":
		...
		rule := agentsec.AllowRule{Command: command, HasArgs: hasArgs}
		ruleStr := rule.String() // "Bash(cmd)" or "Bash(cmd *)"

		// Persist to chat's tool allowlist
		if err := h.chatStore.AddToToolAllowlist(hCtx.ChatID, ruleStr); err != nil {
			logrus.WithError(err).WithFields(logrus.Fields{
				"chatID": hCtx.ChatID,
				"rule":   ruleStr,
			}).Warn("Failed to persist tool allowlist entry from callback")
		} else {
			logrus.WithFields(logrus.Fields{
				"chatID": hCtx.ChatID,
				"rule":   ruleStr,
		...
		if err := h.imPrompter.SubmitDecision(requestID, true, false, ruleStr); err != nil {
		...
		resultText = fmt.Sprintf("✅ Always allowed: `%s`", ruleStr)
		logrus.WithFields(logrus.Fields{
			"request_id": requestID,
			"rule":       ruleStr,
			"action":     subAction,
