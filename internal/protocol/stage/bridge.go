...
func Adapt(next Endpoint, bridge Bridge) (Endpoint, error) {
	...
	if err := checkChainProtocol(source); err != nil {
		return nil, fmt.Errorf("adapt protocol bridge: source: %w", err)
	}
	target := bridge.Target()
	if err := checkChainProtocol(target); err != nil {
		return nil, fmt.Errorf("adapt protocol bridge %q -> ?: target: %w", source, err)
	}
