...
func TestBuildTopologyMixedProtocols(t *testing.T) {
	...
	wantValue := "bridge_chat_beta(bridge_beta_responses(bridge_responses_google(terminal response)))"
	...
	wantCalls := []string{
		...
		"tool_loop:request",
		"bridge_responses_google:request",
		"terminal:request",
		"terminal:response",
		"bridge_responses_google:response",
		"tool_loop:response",
...
func TestBridgeRegistryResolution(t *testing.T) {
	...
	// No implicit identity: a pair resolves only through an explicit entry.
	if _, err := registry.Resolve(protocol.TypeAnthropicBeta, protocol.TypeAnthropicBeta, 0); err == nil {
		t.Fatal("Resolve() synthesized an unregistered same-protocol bridge")
	}
...
func TestBuildTopologyRejectsMissingBridgeBeforeExecution(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), `"anthropic_beta" -> "google": not registered`) {
		t.Fatalf("BuildTopology() error = %v", err)
	}
	if len(calls) != 0 {
		t.Fatalf("BuildTopology() executed chain, calls = %v", calls)
