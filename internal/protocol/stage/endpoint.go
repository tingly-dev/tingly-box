...
// checkChainProtocol rejects protocols that may not appear inside a chain:
// the empty protocol and Anthropic V1, which lives only at the edges (see the
// package documentation).
func checkChainProtocol(api protocol.APIType) error {
	switch api {
	case "":
		return fmt.Errorf("empty protocol")
	case protocol.TypeAnthropicV1:
		return fmt.Errorf("%q is handled at the edges; chain protocols use %q", api, protocol.TypeAnthropicBeta)
	}
	return nil
}
