...
func TestAnthropicToOpenAIChatCompletePreservesTargetUsageWhenWireUsageIsMissing(t *testing.T) {
	t.Parallel()
	...
	targetUsage := protocol.NewTokenUsage(13, 5)
	terminal := &memoryEndpoint{
		api: protocol.TypeOpenAIChat,
		complete: func(context.Context, stage.Call) (*stage.Response, error) {
			return &stage.Response{Value: completion, Usage: targetUsage}, nil
		},
	}
	adapted := mustAdapt(t, terminal, NewBetaToOpenAIChat(ChatOptions{}))
	response, err := adapted.Complete(context.Background(), stage.Call{Request: &anthropic.BetaMessageNewParams{Model: "client-model"}})
	if err != nil {
		t.Fatalf("Complete() error = %v", err)
	}
	if response.Usage != targetUsage {
		t.Fatalf("response usage = %+v, want target usage %+v", response.Usage, targetUsage)
	}
}
...
func TestAnthropicIdentityStageThenOpenAIChatTopology(t *testing.T) {
	...
	if _, ok := identity.request.(*anthropic.BetaMessageNewParams); !ok {
...
func TestAnthropicStreamTreatsZeroUsageAsMissing(t *testing.T) {
	t.Parallel()
...
func TestAnthropicToOpenAIChatFailuresAndCancellation(t *testing.T) {
	...
	t.Run("wrong complete response", func(t *testing.T) {
		...
		adapted := mustAdapt(t, terminal, NewBetaToOpenAIChat(ChatOptions{}))
		_, err := adapted.Complete(context.Background(), stage.Call{Request: &anthropic.BetaMessageNewParams{Model: "m"}})
	...
	t.Run("wrong stream event", func(t *testing.T) {
		...
		adapted := mustAdapt(t, terminal, NewBetaToOpenAIChat(ChatOptions{}))
		stream, err := adapted.Stream(context.Background(), stage.Call{Request: &anthropic.BetaMessageNewParams{Model: "m"}})
		if err != nil {
			t.Fatalf("Stream() error = %v", err)
		}
...
type staticUsageConverter struct {
	usage *protocol.TokenUsage
}

func (staticUsageConverter) Next() (any, bool, error) { return nil, true, nil }

func (c staticUsageConverter) Usage() *protocol.TokenUsage { return c.usage }
