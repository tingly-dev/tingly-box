...
func TestAnthropicBetaGuardrailStageRestoresCredentialsWhenResponseEvaluationFailsOpen(t *testing.T) {
	t.Parallel()

	const secret = "sk-fail-open-secret"
	credential := guardrailscore.ProtectedCredential{
		ID: "credential-fail-open", Name: "test key", Type: guardrailscore.ProtectedCredentialTypeAPIKey,
		Secret: secret, AliasToken: "TINGLY_CRED_API_KEY_FAIL_OPEN", Enabled: true,
	}
	evaluationErr := errors.New("policy backend unavailable")
	runtime := testGuardrailsRuntime(policyRunnerFunc(func(_ context.Context, input guardrailscore.Input) (guardrailscore.Result, error) {
		if input.Direction == guardrailscore.DirectionResponse {
			return guardrailscore.Result{}, evaluationErr
		}
		return guardrailscore.Result{Verdict: guardrailscore.VerdictAllow}, nil
	}))
	runtime.SetCredentialCache(guardrailsruntime.BuildCredentialCache([]guardrailscore.ProtectedCredential{credential}, []string{"anthropic"}))
	...
	request := &anthropic.BetaMessageNewParams{
		...
		Messages: []anthropic.BetaMessageParam{anthropic.NewBetaUserMessage(anthropic.NewBetaTextBlock("use " + secret))},
	}
	terminal := &fakeEndpoint{
		api: protocol.TypeAnthropicBeta,
		complete: func(context.Context, protocolstage.Call) (*protocolstage.Response, error) {
			return &protocolstage.Response{Value: &anthropic.BetaMessage{
				Content: []anthropic.BetaContentBlockUnion{{Type: "text", Text: "use " + credential.AliasToken}},
			}}, nil
		},
	}
	response, err := composeAnthropicBetaGuardrail(t, terminal, runtime).Complete(context.Background(), protocolstage.Call{Request: request})
	if err != nil {
		t.Fatalf("Complete() error = %v, want fail-open response", err)
	}
	if got := response.Value.(*anthropic.BetaMessage).Content[0].Text; got != "use "+secret {
		t.Fatalf("client response text = %q, want restored secret", got)
	}
}
...
func TestAnthropicBetaGuardrailStageRejectsNilCompleteResponse(t *testing.T) {
	t.Parallel()
	...
	terminal := &fakeEndpoint{
		api:      protocol.TypeAnthropicBeta,
		complete: func(context.Context, protocolstage.Call) (*protocolstage.Response, error) { return nil, nil },
	}
	request := &anthropic.BetaMessageNewParams{Model: "client-model", MaxTokens: 64}
	if _, err := composeAnthropicBetaGuardrail(t, terminal, runtime).Complete(context.Background(), protocolstage.Call{Request: request}); err == nil {
		t.Fatal("Complete() error = nil, want nil-response contract error")
	}
}

func TestAnthropicBetaGuardrailPendingEventsHonorCancellation(t *testing.T) {
	t.Parallel()

	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	stream := &anthropicBetaGuardrailStream{
		stream:  &fakeStream{},
		pending: []protocolstage.Event{{Value: "pending"}},
	}
...
func TestNormalizeAnthropicBetaEventRejectsNullPayload(t *testing.T) {
	t.Parallel()

	if _, err := normalizeAnthropicBetaEvent(protocolstream.AnthropicEvent{Type: "message_stop", Data: nil}); err == nil {
		t.Fatal("normalizeAnthropicBetaEvent() error = nil, want object payload error")
	}
}
