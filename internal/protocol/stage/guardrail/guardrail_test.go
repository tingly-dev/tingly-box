...
func TestGuardrailStageRejectsNilDownstreamStream(t *testing.T) {
	t.Parallel()

	guardrail := mustGuardrail(t, Config{Evaluator: &fakeEvaluator{api: protocol.TypeAnthropicBeta}})
	terminal := &fakeEndpoint{
		api: protocol.TypeAnthropicBeta,
		stream: func(context.Context, protocolstage.Call) (protocolstage.EventStream, error) {
			return nil, nil
	...
	if _, err := endpoint.Stream(context.Background(), protocolstage.Call{Request: "request"}); err == nil {
		t.Fatal("Stream() error = nil, want nil-stream contract error")
	}
}

func TestGuardrailStageRejectsNilDownstreamResponse(t *testing.T) {
	t.Parallel()
	...
	if _, err := endpoint.Complete(context.Background(), protocolstage.Call{Request: "request"}); err == nil {
