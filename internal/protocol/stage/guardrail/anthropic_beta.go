...
func (e *anthropicBetaEndpoint) Complete(ctx context.Context, call protocolstage.Call) (*protocolstage.Response, error) {
	...
	if response == nil {
		return nil, fmt.Errorf("Anthropic Beta Guardrail Stage received a nil response")
	}
...
func (s *anthropicBetaGuardrailStream) Next(ctx context.Context) (protocolstage.Event, error) {
	...
	for {
		...
		decision, rewritten, rewriteErr := guardrailsmutate.RewriteAnthropicToolUseEventDecision(s.mask, s.streamState, betaEvent)
		if rewriteErr != nil {
			if s.onError != nil {
		...
		if decision == guardrailsmutate.AnthropicToolUseDecisionNone {
		...
		reason := ""
		if decision == guardrailsmutate.AnthropicToolUseDecisionBlock {
			verdict = VerdictBlock
			reason = "stream tool use rewritten"
		}
		s.parent.report(PhaseEvent, Decision{Verdict: verdict, Reason: reason}, nil)
	}
}
...
func normalizeAnthropicBetaEvent(value any) (*anthropic.BetaRawMessageStreamEventUnion, error) {
	...
	case protocolstream.AnthropicEvent:
		...
		if object == nil {
			return nil, fmt.Errorf("decode converted Anthropic Beta Guardrail event: payload is not an object")
		}
