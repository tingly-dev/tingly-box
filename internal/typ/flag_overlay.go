...
func validateFlagValue(spec FlagSpec, raw json.RawMessage) error {
	...
	case FlagTypeMultiEnum:
		// Comma-separated selection over the spec's options; "" = none.
		var s string
		if err := json.Unmarshal(raw, &s); err != nil {
			return fmt.Errorf("expected a comma-separated string of options")
		}
		for _, part := range strings.Split(s, ",") {
			...
			if !found {
				return fmt.Errorf("value %q is not one of the allowed options", part)
			}
		}
