...
func TestValidateFlagOverlay(t *testing.T) {
	cases := []struct {
		...
		{"multi_enum ok", FlagOverlay{"recording": raw(`"client_request,upstream_request"`)}, ""},
		{"multi_enum empty is inactive", FlagOverlay{"recording": raw(`""`)}, ""},
		{"multi_enum bad option", FlagOverlay{"recording": raw(`"nope"`)}, "not one of the allowed options"},
