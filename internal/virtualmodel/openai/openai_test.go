...
func TestMockModel_Tool_Response(t *testing.T) {
	vm := NewMockModel(&MockModelConfig{
		ID:       "tool-openai",
		ToolCall: &virtualmodel.ToolCallConfig{Name: "web_search", Arguments: map[string]interface{}{"query": "test"}},
	})
	resp, err := vm.HandleOpenAIChat(nil)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
...
func TestMockModel_HandleOpenAIChatStream_ToolModel(t *testing.T) {
	vm := NewMockModel(&MockModelConfig{
		ID:       "tool-openai",
		ToolCall: &virtualmodel.ToolCallConfig{Name: "search", Arguments: map[string]interface{}{"q": "test"}},
	})
	var events []any
