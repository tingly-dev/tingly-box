...
func (ph *ProtocolHandler) tryProtocolStageOpenAIResponses(
	...
	guardrailsEnabled := ph.guardrailsEnabledForProtocolStage(GetTrackingContextScenario(c), protocol.TypeOpenAIResponses)
...
func (ph *ProtocolHandler) serveProtocolStageOpenAIResponsesStream(
	...
	for {
		...
		if errors.Is(nextErr, io.EOF) {
			...
			nextErr = fmt.Errorf("OpenAI Responses Protocol Stage stream ended without a terminal event")
		}
		if nextErr != nil {
		...
		if protocolStageOpenAIResponsesTerminalEvent(eventType) {
			sawTerminalEvent = true
			if protocolStageOpenAIResponsesFailureEvent(eventType) {
				terminalErr = fmt.Errorf("OpenAI Responses stream ended with %s", eventType)
			}
		}
	...
	preserveProtocolStageSideEffectBoundary(c, nil, result.SideEffectsCommitted)
	if result.Usage != nil {
		ph.trackUsageWithTokenUsage(c, result.Usage, terminalErr)
	}
	finalCapture.finish(c.Request.Context())
	return terminalErr
}

func protocolStageOpenAIResponsesFailureEvent(eventType string) bool {
	return eventType == "response.failed" || eventType == "error"
}

func protocolStageOpenAIResponsesTerminalEvent(eventType string) bool {
	switch eventType {
