...
func TestProtocolStageOpenAIResponsesUsagePreservesCacheDetails(t *testing.T) {
	t.Parallel()

	usage := protocolStageOpenAIResponsesUsage([]byte(`{
		"response": {
			"usage": {
				"input_tokens": 10,
				"output_tokens": 4,
	...
	if usage.InputTokens != 7 || usage.OutputTokens != 4 || usage.CacheReadTokens != 3 || usage.CacheWriteTokens != 2 || usage.ReasoningTokens != 1 {
		t.Fatalf("usage = %#v", usage)
	}
}
