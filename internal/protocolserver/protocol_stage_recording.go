...
func (ph *ProtocolHandler) rememberProtocolStageOriginalInput(c *gin.Context, scenario typ.RuleScenario, body []byte) {
	if ph == nil || !ph.deps.ProtocolStageEnabled || ph.deps.Config == nil || c == nil || len(body) == 0 {
		return
	}
	if !ph.deps.Config.GetScenarioConfig(scenario).IsRecordingEnable() {
		return
	}
...
const protocolStageRecordingKey = "protocol_stage_recording"

func enableProtocolStageAttemptTracking(c *gin.Context, recordings ...*protocolStageRequestRecording) {
	if c != nil {
		c.Set(protocolStageRecordingActiveKey, true)
		if len(recordings) > 0 && recordings[0] != nil {
			c.Set(protocolStageRecordingKey, recordings[0])
		}
	}
}

func observeProtocolStageSetupFailure(c *gin.Context, requestErr error) {
	if c == nil || requestErr == nil {
		return
	}
	value, exists := c.Get(protocolStageRecordingKey)
	if !exists {
		return
	}
	recording, ok := value.(*protocolStageRequestRecording)
	if !ok || recording == nil {
		return
	}
	recording.mu.Lock()
	if recording.used {
		recording.lastAttemptErr = requestErr
	}
	recording.mu.Unlock()
}
