...
func (ph *ProtocolHandler) tryProtocolStageOpenAIChat(
	...
	if !protocolStageOpenAIChatChoiceCompatible(target, req.ChatCompletionNewParams) {
		logProtocolStageFallback(c, protocol.TypeOpenAIChat, target, "cross-protocol Chat n > 1 is not representable by the target protocol")
		return false
	}
	...
	defer func() {
		markProtocolStageAttemptFailed(c, requestErr)
		if stageRecording != nil {
			stageRecording.observeAttempt(requestErr)
		}
	}()
	...
	stages = append(stages,
		newProtocolTransformStage(
			...
			scenarioFlags,
			isStreaming,
			protocolStageProviderTransforms(target,
...
func protocolStageOpenAIChatChoiceCompatible(target protocol.APIType, request any) bool {
	return target == protocol.TypeOpenAIChat || assembler.OpenAIChatChoiceCount(request) <= 1
}
...
func protocolStageProviderTransforms(target protocol.APIType, groups ...[]transform.Transform) []transform.Transform {
	result := appendProtocolStageTransforms(groups...)
	if target == protocol.TypeOpenAIChat {
		result = append(result, transform.NewOpenAIChatProviderCleanupTransform())
	}
	return result
}
...
func (ph *ProtocolHandler) serveProtocolStageOpenAIChatStream(
	...
	wrote := false
	expectedChoices := assembler.OpenAIChatChoiceCount(call.Request)
	finishedChoices := make(map[int]struct{})
	for {
		event, nextErr := stream.Next(c.Request.Context())
		if errors.Is(nextErr, io.EOF) {
			if len(finishedChoices) >= expectedChoices {
				break
			}
			nextErr = errors.New("OpenAI Chat Protocol Stage stream ended without a terminal finish_reason")
		}
		if nextErr != nil {
		...
		protocolStageTrackFinishedChatChoices(chunk, expectedChoices, finishedChoices)
		if disableUsage {
			chunk.Usage = nil
...
func protocolStageTrackFinishedChatChoices(chunk wire.ChatStreamChunk, expected int, finished map[int]struct{}) {
	for _, choice := range chunk.Choices {
		if choice.FinishReason != nil && *choice.FinishReason != "" && choice.Index >= 0 && choice.Index < expected {
