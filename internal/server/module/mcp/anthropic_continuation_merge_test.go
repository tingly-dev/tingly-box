...
func TestAnthropicBetaApplyContinuation_MergesIncomingToolResultIntoSameUserMessage(t *testing.T) {
	...
	incoming := &anthropic.BetaMessageNewParams{
		Messages: []anthropic.BetaMessageParam{
			anthropic.NewBetaUserMessage(
				anthropic.NewBetaToolResultBlock("toolu_external", "bash-result", false),
			),
		},
	}
...
func TestAnthropicV1ApplyContinuation_MergesIncomingToolResultIntoSameUserMessage(t *testing.T) {
	...
	incoming := &anthropic.MessageNewParams{
		Messages: []anthropic.MessageParam{
			anthropic.NewUserMessage(
				anthropic.NewToolResultBlock("toolu_external", "bash-result", false),
			),
		},
	}
