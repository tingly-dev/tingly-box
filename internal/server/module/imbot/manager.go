...
func NewBotManager(ctx context.Context, cfg *config.Config) (*BotManager, error) {
	...
	// Enabled bots are started by StartRemoteCoder/StartAllEnabled and kept
	// alive in-place by the internal manager's event-driven restart-on-crash,
	// replacing the former 30s reconciliation poll. The server lifecycle context
	// governs those restarts so shutdown skips any pending restart.
