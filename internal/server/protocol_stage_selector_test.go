...
func TestProtocolStageSelector(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "enabled chat to responses", enabled: true, source: protocol.TypeOpenAIChat, target: protocol.TypeOpenAIResponses, want: true},
		{name: "enabled registered chat identity", enabled: true, source: protocol.TypeOpenAIChat, target: protocol.TypeOpenAIChat, want: true},
