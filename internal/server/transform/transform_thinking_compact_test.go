...
// applyThinkingCompact runs the transform through its real entry point.
func applyThinkingCompact(t *testing.T, keepLastNRounds int, req any) {
	t.Helper()
	transformer := NewThinkingCompactTransform(keepLastNRounds)
	var ctx *protocoltransform.TransformContext
	switch r := req.(type) {
	case *anthropic.MessageNewParams:
		ctx = protocoltransform.NewTransformContext(r)
	case *anthropic.BetaMessageNewParams:
		ctx = protocoltransform.NewTransformContext(r)
	default:
		t.Fatalf("unsupported request type %T", req)
	}
	require.NoError(t, transformer.Apply(ctx))
}
...
func TestRemoveV1ThinkingBlocks_TextOnly(t *testing.T) {
	...
	result, _ := removeV1ThinkingBlocks(content, 0)
...
func TestRemoveV1ThinkingBlocks_WithThinking(t *testing.T) {
	...
	result, count := removeV1ThinkingBlocks(content, 0)
...
func TestRemoveBetaThinkingBlocks_WithThinking(t *testing.T) {
	...
	result, count := removeBetaThinkingBlocks(content, 0)
...
func TestThinkingCompact_Name(t *testing.T) {
	assert.Equal(t, "compact_thinking", NewThinkingCompactTransform(1).Name())
}

func TestThinkingCompactV1_RemovesThinkingFromPastRounds(t *testing.T) {
...
func TestThinkingCompactV1_EmptyMessages(t *testing.T) {
	req := &anthropic.MessageNewParams{
		Model:     anthropic.Model("claude-3-5-sonnet-20241022"),
		MaxTokens: 1024,
		Messages:  []anthropic.MessageParam{},
...
func TestThinkingCompactV1_NilMessages(t *testing.T) {
	req := &anthropic.MessageNewParams{
		Model:     anthropic.Model("claude-3-5-sonnet-20241022"),
		MaxTokens: 1024,
		Messages:  nil,
...
func TestThinkingCompactV1_WithToolUse(t *testing.T) {
	req := &anthropic.MessageNewParams{
		Model:     anthropic.Model("claude-3-5-sonnet-20241022"),
		MaxTokens: 1024,
		Messages: []anthropic.MessageParam{
...
func TestThinkingCompactBeta_RemovesThinkingFromPastRounds(t *testing.T) {
...
func TestThinkingCompactBeta_WithToolUse(t *testing.T) {
	req := &anthropic.BetaMessageNewParams{
		Model:     anthropic.Model("claude-3-5-sonnet-20241022"),
		MaxTokens: 1024,
		Messages: []anthropic.BetaMessageParam{
...
func TestThinkingCompactV1_SingleRound(t *testing.T) {
	req := &anthropic.MessageNewParams{
		Model:     anthropic.Model("claude-3-5-sonnet-20241022"),
		MaxTokens: 1024,
		Messages: []anthropic.MessageParam{
...
// TestThinkingCompactV1_ComplexToolUseFlow tests the complete flow with tool results
func TestThinkingCompactV1_ComplexToolUseFlow(t *testing.T) {
	req := &anthropic.MessageNewParams{
		Model:     anthropic.Model("claude-3-5-sonnet-20241022"),
		MaxTokens: 1024,
		Messages: []anthropic.MessageParam{
...
// TestThinkingCompactV1_KeepLastNRounds verifies that k=2 preserves the last 2 rounds' thinking
func TestThinkingCompactV1_KeepLastNRounds(t *testing.T) {
	req := &anthropic.MessageNewParams{
		Model:     anthropic.Model("claude-3-5-sonnet-20241022"),
		MaxTokens: 1024,
		Messages: []anthropic.MessageParam{
	...
	applyThinkingCompact(t, 2, req) // Keep last 2 rounds
	require.Len(t, req.Messages, 6)
...
// TestThinkingCompact_NonAnthropicPassthrough verifies unsupported request
// types pass through untouched.
func TestThinkingCompact_NonAnthropicPassthrough(t *testing.T) {
	transformer := NewThinkingCompactTransform(1)
	// A GoogleRequest is in the chain's request union but not handled by this
	// transform; Apply must be a no-op rather than an error.
	ctx := &protocoltransform.TransformContext{Request: struct{ any }{}}
	require.NoError(t, transformer.Apply(ctx))
}
