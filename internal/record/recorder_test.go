...
func TestRecorderRejectsTypedNilWithoutPanicking(t *testing.T) {
	recorder := newRecordingTestRecorder(t, protocol.TypeOpenAIChat)
	var response *rawJSONValue
	require.NotPanics(t, func() {
		require.ErrorContains(t, recorder.SetFinalResponse(protocol.TypeOpenAIChat, response), "nil")
	})
}

func TestRecorderDoesNotSerializeAfterFinish(t *testing.T) {
	recorder := newRecordingTestRecorder(t, protocol.TypeOpenAIChat)
	_, first := recorder.Finish(nil)
	require.True(t, first)

	calls := 0
	value := countingMarshaler{calls: &calls}
	_, err := recorder.BeginExchange(ExchangeMetadata{Protocol: protocol.TypeOpenAIChat}, value)
	require.ErrorIs(t, err, ErrFinished)
	require.ErrorIs(t, recorder.SetFinalResponse(protocol.TypeOpenAIChat, value), ErrFinished)
	require.Zero(t, calls)
}
