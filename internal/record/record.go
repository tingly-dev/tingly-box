...
// Project returns an immutable copy suitable for a recording mode that may
// omit response payloads. Provider requests and the original input remain part
// of every RequestRecord; response inclusion is controlled independently.
func (r *RequestRecord) Project(includeProviderResponses, includeFinalResponse bool) *RequestRecord {
	if r == nil {
		return nil
	}
	projected := cloneRequestRecord(*r)
	if !includeProviderResponses {
		for index := range projected.ProviderExchanges {
			projected.ProviderExchanges[index].Response = nil
		}
	}
	if !includeFinalResponse {
		projected.FinalResponse = nil
	}
	return &projected
}
