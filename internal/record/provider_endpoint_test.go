...
func TestObserveProviderStreamEarlyCloseFailsWithoutPartialResponse(t *testing.T) {
	recorder := newRecordingTestRecorder(t, protocol.TypeAnthropicBeta)
	providerStream := &recordingTestStream{events: []stage.Event{
		{Value: json.RawMessage(`{"type":"message_start","message":{"id":"partial","type":"message","role":"assistant","content":[],"model":"provider-model","stop_reason":null,"stop_sequence":null,"usage":{"input_tokens":1,"output_tokens":0}}}`)},
	}}
	endpoint := &recordingTestEndpoint{
		api: protocol.TypeAnthropicBeta,
		stream: func(context.Context, stage.Call) (stage.EventStream, error) {
	...
	stream, err := ObserveProvider(endpoint, recorder, ExchangeMetadata{Attempt: 1}).Stream(
		context.Background(),
		stage.Call{Request: map[string]any{"model": "provider-model"}},
	)
	require.NoError(t, err)
	...
	completed, first := recorder.Finish(errors.New("client stopped"))
	require.True(t, first)
	require.Len(t, completed.ProviderExchanges, 1)
	require.Equal(t, OutcomeFailed, completed.ProviderExchanges[0].Outcome)
	require.ErrorContains(t, errors.New(completed.ProviderExchanges[0].Error), "before a terminal event")
	require.Nil(t, completed.ProviderExchanges[0].Response)
}

func TestObserveProviderResponsesFailedPreservesResponseAndFailure(t *testing.T) {
	recorder := newRecordingTestRecorder(t, protocol.TypeOpenAIResponses)
	providerStream := &recordingTestStream{events: []stage.Event{
		{Value: json.RawMessage(`{"type":"response.failed","sequence_number":1,"response":{"id":"resp-failed","object":"response","status":"failed","model":"provider-model","output":[],"error":{"code":"server_error","message":"provider failed"}}}`)},
	}}
	endpoint := &recordingTestEndpoint{
	...
	completed, first := recorder.Finish(errors.New("provider failed"))
	require.True(t, first)
	require.Len(t, completed.ProviderExchanges, 1)
	exchange := completed.ProviderExchanges[0]
	require.Equal(t, OutcomeFailed, exchange.Outcome)
	require.ErrorContains(t, errors.New(exchange.Error), "response.failed")
	require.NotNil(t, exchange.Response)
	require.Contains(t, string(exchange.Response.Body), `"id":"resp-failed"`)
	require.Contains(t, string(exchange.Response.Body), `"status":"failed"`)
}

func TestObserveProviderStreamAssemblyErrorDoesNotBecomeSuccess(t *testing.T) {
	recorder := newRecordingTestRecorder(t, protocol.TypeOpenAIChat)
	stop := "stop"
	providerStream := &recordingTestStream{events: []stage.Event{
		{Value: map[string]any{
			"id": "chat-a", "object": "chat.completion.chunk",
			"choices": []map[string]any{{"index": 0, "delta": map[string]any{"content": "partial"}}},
		}},
		{Value: map[string]any{
			"id": "chat-b", "object": "chat.completion.chunk",
	...
	completed, first := recorder.Finish(errors.New("recording assembly failed"))
	require.True(t, first)
	require.Len(t, completed.ProviderExchanges, 1)
	exchange := completed.ProviderExchanges[0]
	require.Equal(t, OutcomeFailed, exchange.Outcome)
	require.Contains(t, exchange.Error, "accumulate OpenAI Chat")
	require.Nil(t, exchange.Response)
}
