...
type CodexParams struct {
	...
	// Context1M maps a model name to whether its routing rule requested the 1M
	// context window. Flagged models get a larger context_window in the
	// rendered catalog. nil/missing → default window.
...
func (c *CodexConfig) Apply(paramsInterface interface{}) (*ApplyAgentResult, error) {
	...
	if params.AuthMode != serverconfig.CodexAuthChatGPT {
		configResult, err := serverconfig.ApplyCodexConfig(params.CodexBaseURL, params.Models, params.Prefs, params.WriteCatalog, params.Context1M)
		if err != nil {
