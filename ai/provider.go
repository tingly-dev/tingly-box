...
// e.g. Ollama, or a local model server you wrote yourself). Unlike
// VModelSentinelToken this value IS transmitted, as an Authorization/x-api-key
// header the receiving side is expected to ignore. It exists because some
// client SDKs (anthropic-sdk-go) treat an empty API key as "look for ambient
// credentials" and fail loudly when none are found, instead of just sending an
// empty/absent header the way the OpenAI client does.
